...
### 원칙: 기반 시스템 구현 → 렌더링 전환 → AI 연동 → 문서 수정

구현 중 설계가 변경될 수 있으므로, 문서 수정은 **각 단계 구현 완료 후** 확정된 아키텍처에 맞춰 진행한다.

| 순서 | 대상 | 수정 항목 | 이유 |
|------|------|----------|------|
| **1** | 기반 시스템 구현 | G.1 컴포넌트-인스턴스 + G.2 변수 참조 | 디자인 킷(G.4)과 AI 도구(H)의 선행 의존 |
| **2** | `docs/WASM.md` Phase 0-6 구현 | 렌더링 코드 작업 | 아키텍처 확정 필요 |
| **3** | `docs/AI.md` 업데이트 구현 | A1–A6 (6건) + G.3 시각 피드백 | G.1/G.2 확정 후 AI 도구 정의 |
...
| **6** | 디자인 킷 + 시각 피드백 | G.4 킷 구현 + G.3 피드백 구현 | G.1/G.2 완료 후 진행 |
...
순서 1: G.1 컴포넌트-인스턴스 ──┐
        G.2 변수 참조 시스템 ──┤
                               ├──→ 순서 3: AI.md (A1–A6)
순서 2: WASM.md Phase 0-6 ─────┤   순서 6: G.4 디자인 킷
                               │           G.3 시각 피드백
                               ├──→ 순서 4: LAYOUT_REQUIREMENTS.md (L1–L5)
                               └──→ 순서 5: COMPONENT_SPEC_ARCHITECTURE.md (C1–C18)
```

**이점:**
- G.1/G.2가 AI 도구 정의와 디자인 킷의 선행 조건이므로 먼저 구현
- 렌더링 전환(WASM.md)은 G.1/G.2와 독립적으로 병행 가능
- 문서 수정은 각 단계 완료 후 확정된 코드 기준으로 진행
