...
### Phase 1 — `themes` adapter (Partial Implemented, 2026-04-25 ~ 2026-04-27)

**완료 영역** (`themes` 한정):

- ✅ `apps/builder/src/adapters/canonical/themesAdapter.ts` — `snapshotThemesFromConfig()` + `readCanonicalThemes()` + `ThemeSnapshot` 인터페이스 (R2 확장 슬롯 `customTokens` 포함)
- ✅ `apps/builder/src/adapters/canonical/__tests__/themes.test.ts` — adapter 단위 테스트 (snapshot round-trip + 잘못된 stub 형식 → undefined 처리)
- ✅ `apps/builder/src/adapters/canonical/index.ts:238` — `legacyToCanonical()` 호출 시 `themesSnapshot` 주입 통합
- ✅ ADR-021 `themeConfigStore` 무수정 — call-time 직렬화 (R4 stale snapshot 방지)
- ✅ Phase 1 G-A 통과 조건 (a)~(d) 부분 충족: `themes` 한정으로 PASS

**미완 영역 (Phase 1 G-A 잔여)**:

- ⚠️ `packages/shared/src/types/composition-document.types.ts:284` — `themes?: Record<string, string[]>` stub 유지 (`ThemeSnapshot` 으로 전환 미완, adapter 가 raw cast 로 우회 중)
- ❌ `variables` snapshot adapter 미착수 — `snapshotVariablesFromConfig()` / `readCanonicalVariables()` / `VariablesSnapshot` 타입 0건
- ❌ `apps/builder/src/adapters/canonical/__tests__/variables.test.ts` 미생성
- ❌ `legacyToCanonical()` `variablesSnapshot` 주입 미통합
- ❌ `docs/adr/design/910-canonical-themes-variables-land-plan-breakdown.md` 미생성 (구현 상세 분리 미완)
