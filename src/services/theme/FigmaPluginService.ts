...
export class FigmaPluginService {
  ...
  private static generateCode(
    ...
    if (options.exportTargets.colors && colorTokens.length > 0) {
      ...
      lines.push(
        '      const colorTokens = msg.tokens.filter((t: { type: string }) => t.type === "color");'
      );
    ...
    if (options.exportTargets.textStyles && typographyTokens.length > 0) {
      ...
      lines.push(
        '      const textTokens = msg.tokens.filter((t: { type: string }) => t.type === "typography");'
      );
      ...
      lines.push(
        "          style.fontSize = parseFloat(token.value.fontSize);"
      );
      lines.push("        }");
      ...
      lines.push(
        '          style.lineHeight = { value: parseFloat(token.value.lineHeight), unit: "PIXELS" };'
      );
      lines.push("        }");
      lines.push("        imported++;");
      lines.push("      }");
    ...
    if (options.exportTargets.effects && shadowTokens.length > 0) {
      ...
      lines.push(
        '      const shadowTokens = msg.tokens.filter((t: { type: string }) => t.type === "shadow");'
      );
      ...
      lines.push(
        "          offset: { x: token.value.offsetX || 0, y: token.value.offsetY || 0 },"
      );
...
<body>
   ...
   * Plugin 파일 다운로드 (zip)
   */
  static async downloadPluginFiles(
    result: FigmaPluginExportResult
  ): Promise<void> {
