...
## 현재 composition 구현 분석

composition는 **Quill 에디터 + DOM 오버레이** 방식을 사용한다 (Pencil 패턴 참조).
...
#### 1a. Quill 에디터 오버레이 (Pencil Desktop, **composition 현재**)
...
- **사용처**: Pencil Desktop v1.1.10, composition (현재)
...
- **단점**: composition는 Skia/WebGL 기반 → SVG 전환 불가능
...
- [Fabric.js IText](https://github.com/fabricjs/fabric.js) — Hidden textarea + Canvas 네이티브 커서/셀렉션 렌더링 (가장 성숙한 오픈소스 구현)
- [Hufe921/canvas-editor](https://github.com/Hufe921/canvas-editor) — 14.6k stars, Canvas 위 풀 문서 에디터 (hidden textarea, zero deps, 2900+ lines)
...
- [infinitecanvas.cc Lesson 15-16](https://infinitecanvas.cc/guide/lesson-015) — CanvasKit/Skia 기반 텍스트 렌더링 + 편집 튜토리얼
...
- [Microsoft Edge Blog: Custom web editing with EditContext](https://blogs.windows.com/msedgedev/2024/02/13/custom-web-editing-experiences-with-editcontext/)
- [GitHub: Hidden textarea 비교 논의](https://github.com/w3c/edit-context/issues/102)
- [Can I Use: EditContext](https://caniuse.com/mdn-api_editcontext) — Chrome/Edge 121+ 전용 (Firefox positive signal, Safari unknown)
...
## 부록: CanvasKit Paragraph API 활용 가능성

composition가 CanvasKit/Skia를 사용하므로, Hidden textarea 방식(방안 2) 채택 시 CanvasKit Paragraph API가 커서/셀렉션 구현을 크게 단순화한다:

```typescript
// 클릭 위치 → 커서 위치 (글자 인덱스)
const pos = paragraph.getGlyphPositionAtCoordinate(localX, localY);
const caretIndex = pos.pos; // 텍스트 내 글자 인덱스

// 셀렉션 범위 → 하이라이트 사각형들
const rects = paragraph.getRectsForRange(
  selectionStart,
  selectionEnd,
  { heightStyle: HeightStyle.Max, widthStyle: WidthStyle.Tight }
);
// rects: Array<{ rect: [x, y, w, h], dir: TextDirection }>

// 커서 위치 → 깜빡이는 캐럿 사각형
const caretRect = paragraph.getRectsForRange(caretIndex, caretIndex + 1, ...);
```

이 API들이 있으므로 "문자 폭 수동 측정 → 클릭 위치 매핑" 같은 저수준 작업이 불필요하다. 다만 커서 깜빡임 렌더링, 드래그 셀렉션 UI, 키보드 네비게이션(Home/End/Arrow 등)은 여전히 직접 구현해야 한다.
