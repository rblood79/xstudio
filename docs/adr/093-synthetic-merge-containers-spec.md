...
1. **orientation-based flexDirection runtime 로직은 유지** — `parentProps.orientation` 이 runtime 값이라 spec 정적 선언 불가. spec.containerStyles 는 **기본값만** (orientation=horizontal/vertical 기본), 분기는 orientation fork 유지
2. **TagList runtime 로직 (maxRows, Tag whiteSpace injection) 유지** — 자식 Element 조작/수량 조정은 spec 커버 영역 아님
3. **Factory element 자동 생성 호환 유지** — 기존 저장 데이터의 TagList/RadioItems/CheckboxItems element 그대로 동작
4. **본 ADR scope = spec 신설 + 기본 layout primitive containerStyles 리프팅**. maxRows/injection/orientation fork 는 분기 유지
5. `pnpm type-check` 3/3 + specs 166/166 + builder 217/217 PASS
...
- **Phase 1 (1세션)**:
  1. `TagList.spec.ts` 신설 — archetype:"simple", skipCSSGeneration:true, containerStyles (display:flex, flexWrap:wrap), sizes.md (gap:4)
  2. `RadioItems.spec.ts` 신설 — archetype:"simple", skipCSSGeneration:true, containerStyles (display:flex, flexDirection:column default), sizes.sm/md/lg (gap:8/12/16)
  3. `CheckboxItems.spec.ts` 신설 — RadioItems 와 동일 구조
  4. `components/index.ts` export 추가
  5. 부모 spec childSpecs 배선 (TagGroupSpec.childSpecs=[TagListSpec], etc)
- **Phase 2 (0.5세션)**: `implicitStyles.ts` 분기 3개 리팩토링:
  - taglist 분기: display/flexWrap/gap 기본값을 spec 참조로 전환. **runtime fork 유지**: (a) `orientation` 에 따른 flexDirection (column/row), (b) `labelPosition="side"` 시 flex:1/minWidth:0 주입, (c) maxRows/whiteSpace 자식 injection
  - radioitems/checkboxitems 분기: gap 하드코딩 `sm:8/md:12/lg:16` 제거 → `spec.sizes[sizeName].gap` 참조. **runtime fork 유지**: (a) `orientation` 에 따른 flexDirection (horizontal:row, vertical:column), (b) `alignItems:center` horizontal 시 주입
...
- `rg "gap = sizeName === \"sm\" \\? 8" implicitStyles.ts` = 0건 (RadioItems/CheckboxItems 하드코딩 제거)
- orientation runtime fork 유지 확인 (horizontal/vertical 양쪽 Chrome MCP 실측)
