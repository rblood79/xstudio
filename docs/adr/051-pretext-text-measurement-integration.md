...
**변경 포인트**: `cssResolver.ts` L83 — `overflowWrap: "normal"` → `"break-word"`

이 1줄 변경으로 Pretext SSOT 전면 도입이 가능해진다.
...
**위험 완화**: `normal` 프리셋은 "Normal" Text Behavior로 유지 → 사용자가 명시적으로 오버플로 허용 가능.
...
현재 `TextMeasureResult`에 `lineCount` 추가:
...
| **순 번들 변화**        | —                   | +15KB (Pretext) - 5KB (preprocessBreakWordText 제거) = **+10KB** |
...
| 파일                         | 변경 내용                                                 | Phase |
| ---------------------------- | --------------------------------------------------------- | ----- |
| `cssResolver.ts`             | `overflowWrap: "normal"` → `"break-word"` (1줄)           | 0     |
| `textMeasure.ts`             | `TextMeasureResult.lineCount?` 추가, PretextMeasurer 구현 | A     |
...
| `useTextEdit.ts`             | Pretext 기반 오버레이 크기                                | D-2   |
| `TextEditOverlay.tsx`        | Pretext height 적용                                       | D-2   |
| `specTextStyleForOverlay.ts` | lineHeight 통일                                           | D-2   |
...
- [ ] `overflow-wrap: break-word` 기본값 전환 (cssResolver.ts 1줄)
- [ ] 기본값 전환 후 회귀 테스트 통과
- [ ] Canvas 2D + WebGL 공존 확인
- [ ] 폰트 로딩 이중 감지 작동
