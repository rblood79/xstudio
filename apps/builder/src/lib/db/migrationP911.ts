 ...
 *
 * legacy `LayoutTemplate` (`type="Slot"` 기반) → canonical reusable `FrameNode`
 ...
 * - `type="Slot"` 자식 → 제거 (slot 정보는 `FrameNode.slot` field 로 흡수)
...
/**
 * `LayoutTemplateElement[]` 에서 `type="Slot"` 노드 제거 + 나머지 구조 보존.
 * Slot 위치는 `FrameNode.slot` field + 후속 `descendants` key 로 대체됨.
 *
 * @returns canonical `CanonicalNode[]` (type="Slot" 제외)
 */
