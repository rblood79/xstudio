---
description: UI 컴포넌트 구현/수정 시 React Aria MCP 문서 참조 규칙
globs:
...
# React Aria MCP 연동 규칙

## 컴포넌트 구현 시 React Aria 문서 참조 (MCP)

새 UI 컴포넌트를 구현하거나 기존 컴포넌트를 수정할 때, **React Aria MCP 도구**를 활용하여 공식 API와 접근성 패턴을 참조한다.

### 필수 참조 시점

1. **새 컴포넌트 생성** — 해당 컴포넌트의 React Aria 페이지를 먼저 조회
...
```
1. mcp__react-aria__list_react_aria_pages()          → 관련 페이지 존재 확인
2. mcp__react-aria__get_react_aria_page_info(page)   → 섹션 구조 파악
3. mcp__react-aria__get_react_aria_page(page, section) → API/예제 코드 참조
```
