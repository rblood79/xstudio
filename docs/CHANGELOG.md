...
- ADR-911 명칭 충돌을 정리했다.
  ...
  - Shared export schema/element utilities 의 내부 legacy helper naming 을 mirror terminology 로 정리했다. `layout_id`/`slot_name` mirror field 는 현재 export schema 경계에만 유지한다.
  ...
  - Canvas drag/drop helper, BuilderCanvas layout/frame memo, FramesTab, PageLayoutSelector, ComponentsPanel 의 visible/hot read path 는 `selectCanonicalDocument()` projection rebuild 대신 active canonical document 를 사용한다. 초기 hydration 전 UI 목록은 `layouts` fallback 으로 유지하되 projection rebuild 는 수행하지 않는다.
  - BuilderCore refresh/theme/publish, `usePageManager` project hydrate, `elementCreation` history/reorder, `layoutActions` slot read path 의 caller-level `selectCanonicalDocument()` projection rebuild 를 제거했다. 해당 경로는 active canonical document, project layout snapshot, frame mirror adapter 를 사용한다.
...
**Note**: 2025년 이전 변경 이력은 [CHANGELOG-2025-archived.md](CHANGELOG-2025-archived.md)를 참조하세요.
