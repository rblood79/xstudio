...
| R2  | 11 주대상 전수 fixture 회귀에서 N_edited > 0 인 기존 프로젝트의 시각 변화가 BC 회귀로 오인                                                  |  MED   | 상세는 R2 상세 섹션 (표 아래) 참조                                                                                                     |
...
- **Phase 0 실측 3 컴포넌트**: `GridList` / `Table` / `TagGroup` — Supabase 쿼리 또는 export scan 으로 `N_edited` 실측 (ADR-906 breakdown Phase 0 의 SQL 재사용).
- **Phase 0 가정 8 컴포넌트**: `Breadcrumbs, ComboBox, ListBox, Menu, Select, Tabs, Toolbar, Tree` — self-render items SSOT 가 이미 확립 (ADR-066/068/073/076/078) 또는 root `style` 편집 UI 접근 제한 → `N_edited = 0` **가정 기록** (breakdown matrix 에 "가정" 명시). 해당 컴포넌트의 Phase 4 follow-up ADR 에서 실측 승격이 필수.
- **G1 audit matrix 산식**: 11 주대상 × 3 축 (a/b/c) + N_edited 11 cell (실측 3 + 가정 기록 8) = **44 cell** (원래 48 cell 에서 TagList 행 제거 + Menu 편입으로 재산출). Phase 0 은 44 cell 전부 기록되어야 통과이며, N_edited 8 가정 cell 도 "가정" 라벨로 breakdown 에 기재.
- Baseline 분기:
  - (a) **unedited baseline** (`style` 부재 또는 padding/gap 미설정): 현 `main` 렌더와 **byte-equal** 유지. fixture test 로 Phase 3/4/5 각 컴포넌트에 고정.
  - (b) **edited baseline** (`N_edited > 0` 인 GridList/Table/TagGroup): 편집값이 DOM/Skia 양쪽에 정확히 반영되는지 시각 equivalence 검증. 이는 BC 회귀가 아니라 **Hard Constraint 1 충족**.
- G8 통과 조건 (Phase 6): unedited baseline 회귀 0 + edited baseline 시각 equivalence PASS. Phase 4 follow-up ADR 에서 N_edited 실측이 0 이 아닌 경우 edited baseline 추가.

## Gates

| Gate                              | 시점                     | 통과 조건                                                                                                                                                                                                                                                                                                                                                                                                                          | 실패 시 대안                                                                      |
| --------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| G1: Audit Matrix                  | Phase 0                  | 11 주대상 컴포넌트 audit matrix 총 **44 cell** (11 × 3 축 (a/b/c) + 11 × N_edited, 단 N_edited 는 3 실측 + 8 가정 기록) 이 breakdown 에 기록되고 Phase 3/4/5 적용 우선순위 결정. 추가로 `packages/specs/vitest.config.ts` 및 `packages/shared/vitest.config.ts` 의 include 패턴에 Phase 1-2 신규 test 경로가 수집됨을 확증 (test discovery 확인).                                                                                  | scope 축소 — Phase 4 에서 적용할 컴포넌트 집합 재정의                             |
