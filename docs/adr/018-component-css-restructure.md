...
→ utilities.css의 `[data-variant]` 셀렉터와 비호환. Phase 5에서 **Card.css 셀렉터를 `[data-variant]`로 전환** 필요. Card.tsx는 이미 `data-variant` 속성을 전달하고 있으므로(L162) **CSS 변경만으로 완료** — React 컴포넌트 수정 불필요.
...
6. **Card.css 셀렉터 전환**: 클래스 기반 `.primary` → `[data-variant="primary"]` CSS만 전환 (Card.tsx는 이미 data-variant 전달)
...
| RadioGroup.css  |     24     |    ~15     |  -38%  |
| Switch.css      |    230     |    ~60     |  -74%  |
| Slider.css      |    339     |    ~80     |  -76%  |
| ProgressBar.css |    125     |    ~35     |  -72%  |
| Meter.css       |    111     |    ~35     |  -68%  |
...
- **CSS만 변경**: `&.primary` → `&:where([data-variant="primary"])`, `&.elevated` → `&:where([data-variant="elevated"])` 등
- **React 변경 불필요**: Card.tsx:162에서 `'data-variant': variant`을 **이미 전달 중**. CSS 셀렉터 전환만으로 충분
- `.sm`/`.md`/`.lg` 사이즈도 동일하게 `[data-size]` 셀렉터로 전환 (Card.tsx:163에서 `'data-size': size` 이미 전달)
...
4. **Card.css 셀렉터 전환** — 클래스 기반(`.primary`) → `[data-variant]` 기반 전환 (CSS만 변경, Card.tsx는 이미 data-variant 전달 중)
