...
| Slot section base wiring | Properties ##Slot section## frame-only UI 를 연결. `Frame.slot: false | string[]` 를 enable/disable 하고 reusable origin id 를 recommended component 로 add/remove. IndexedDB top-level `slot` 과 `metadata.slot` backup 을 함께 보존하며 sanitizer 에서 canonical `slot` 유실을 방지 | G4-F/G4-H 부분 완료 |
