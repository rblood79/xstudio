...
```
.workspace (position: fixed, overflow: hidden)
└─ .canvas-container (position: absolute; inset: 0)
   └─ <canvas data-testid="skia-canvas-unified" z-index: 2, pointer-events: auto>
```

`.canvas-container` 는 Skia canvas의 직접 부모이며, 여기에 형제 DOM을 추가해도 Skia 렌더 파이프라인에 영향을 주지 않는다.
...
- `useViewportSyncStore` 를 React 컴포넌트에서 처음 구독하는 경로가 추가됨 (기존에는 Skia RAF에서 `viewportState` 직접 읽기) — 구독 비용 자체는 미미하나 스토어 책임 범위가 확장됨을 명시 필요
- Safari 대응을 위해 `mask-image` 와 `-webkit-mask-image` 이중 선언 필요

## References

- [Stitch (Google) React Flow 배경 패턴 분석](# "이 conversation 내 수행 — 2레이어 SVG + mask-image 조합")
