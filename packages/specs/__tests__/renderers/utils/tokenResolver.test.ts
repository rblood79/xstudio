...
describe("resolveToken", () => {
  describe("color 카테고리", () => {
    it("light 테마에서 올바른 색상 반환", () => {
      expect(resolveToken("{color.primary}" as TokenRef, "light")).toBe(
        "#2563eb",
      );
    });

    it("dark 테마에서 올바른 색상 반환", () => {
      expect(resolveToken("{color.primary}" as TokenRef, "dark")).toBe(
        "#3b82f6",
      );
    });

    it("on-primary 등 하이픈 포함 이름", () => {
      expect(resolveToken("{color.on-primary}" as TokenRef, "light")).toBe(
        "#ffffff",
      );
    });
  });
  ...
  describe("typography 카테고리", () => {
    it("올바른 값 반환", () => {
      expect(resolveToken("{typography.text-sm}" as TokenRef, "light")).toBe(
        14,
      );
    });
  });
  ...
  it("테마 미지정 시 light 기본값", () => {
    expect(resolveToken("{color.primary}" as TokenRef)).toBe("#2563eb");
  });
});

describe("resolveColor", () => {
  it("TokenRef → 색상값 반환", () => {
    expect(resolveColor("{color.primary}" as TokenRef, "light")).toBe(
      "#2563eb",
    );
  });
...
describe("tokenToCSSVar", () => {
  it("color → 시맨틱 CSS 변수", () => {
    expect(tokenToCSSVar("{color.primary}" as TokenRef)).toBe(
      "var(--highlight-background)",
    );
    expect(tokenToCSSVar("{color.on-primary}" as TokenRef)).toBe(
  ...
  it("typography → var(--name)", () => {
    expect(tokenToCSSVar("{typography.text-sm}" as TokenRef)).toBe(
      "var(--text-sm)",
    );
  });
