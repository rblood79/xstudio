...
} from "../../src/renderers/ReactRenderer";
...
describe("renderToReact", () => {
  ...
  it("무효한 variant → console.warn", () => {
    ...
    renderToReact(ButtonSpec, {
      variant: "nonexistent" as ButtonProps["variant"],
    } as ButtonProps);
...
describe("generateCSSVariables", () => {
  it("primary variant → 올바른 CSS 변수 생성", () => {
    ...
    expect(vars["--spec-bg"]).toBe("var(--highlight-background)");
    expect(vars["--spec-bg-hover"]).toBe(
      "color-mix(in srgb, var(--highlight-background) 85%, black)",
    );
    expect(vars["--spec-bg-pressed"]).toBe(
      "var(--highlight-background-pressed)",
    );
    expect(vars["--spec-text"]).toBe("var(--highlight-foreground)");
  });
  ...
  it("border 있는 variant → --spec-border 포함", () => {
    ...
    expect(vars["--spec-border"]).toBe("var(--border-color)");
  });
