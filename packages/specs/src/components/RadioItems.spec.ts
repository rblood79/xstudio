 ...
 * - CSS 자동 생성: skipCSSGeneration: true — 부모 RadioGroupSpec.childSpecs 경로로
 *   부모 RadioGroup generated CSS 내부에 inline emit (ADR-078/090/092 패턴 재사용).
 * - Skia consumer: ADR-094 `expandChildSpecs` 자동 등록 → `TAG_SPEC_MAP` /
 *   `LOWERCASE_TAG_SPEC_MAP` / `tagToElement.ts` 모든 소비처 자동 혜택.
 * - render.shapes: () => [] — RadioItems 자체 시각 없음. Radio 자식이 각자 렌더링.
 *
 * containerStyles: `implicitStyles.ts:882-893` RadioItems 분기의 display/flexDirection
 *   base primitive 를 Spec SSOT 로 리프팅.
 *   Hard Constraint #2: RadioGroup orientation 기본값 vertical → column 기본 (RadioGroup.spec.ts:35).
 *
 * sizes: sm/md/lg gap 8/12/16 — size-indexed (implicitStyles.ts:888 기존값 일치).
 ...
 * RadioItems Props
 *
 * RadioItems 는 RadioGroup 내부의 중간 컨테이너로 composition 이 자동 생성하는 element.
 * RAC 표준 API 에 없는 composition 자체 추상화.
 */
export interface RadioItemsProps {
  // 부모 RadioGroup 으로부터 propagation 수신 전용
  size?: "sm" | "md" | "lg";
 ...
 *   부모 RadioGroupSpec.childSpecs 경로로 부모 generated CSS 에만 inline emit.
 * render.shapes: () => [] — Skia shapes 없음 (container shell).
 *
 * containerStyles: ADR-093 — RadioItems 분기 base primitive 리프팅.
 *   Hard Constraint #2: RadioGroup orientation 기본값 vertical → column 기본.
 ...
 *   orientation="horizontal" 시 flexDirection:"row" + alignItems:"center" override.
 */
export const RadioItemsSpec: ComponentSpec<RadioItemsProps> = {
  name: "RadioItems",
  description:
    "RadioGroup 내부 중간 컨테이너 (composition 자체 추상) — display:flex column default",
  archetype: "simple",
  element: "div",
  ...
  // ADR-093 Phase 1: implicitStyles.ts RadioItems 분기(display/flexDirection base)
  //   를 Spec SSOT 로 리프팅. ADR-094 `expandChildSpecs` 를 통해 자동 주입.
  //   Hard Constraint #2: RadioGroup orientation 기본 vertical → column.
  ...
  // sizes: sm/md/lg gap 8/12/16 (implicitStyles.ts:888 기존값 일치).
  //   borderRadius: "{radius.none}" — CSS Generator undefined 출력 방지 (ADR-092 실수 방지).
  sizes: {
    sm: {
      height: 0,
  ...
  render: {
    ...
    //   RadioItems 시각 없음. Radio 자식이 각자 렌더링.
    shapes: () => [],
    react: () => ({}),
  },
};
