...
XStudio의 Card 컴포넌트는 초기 구현 시 React Aria 기반으로 자체 설계되었다. 이후 ADR-022/023에서 S2 토큰/variant 체계로 전환했으나, **Spec(S2 네이밍)과 React/CSS(레거시 네이밍)가 불일치**하며, S2의 핵심 설계 패턴(슬롯 기반 컴포지션, Card 변형, CardPreview)이 미도입 상태다.
...
XStudio의 구조적 자식 모드(CardHeader/CardContent)와 유사하지만, S2는 `slot` 속성으로 역할을 명시하는 점이 다르다. XStudio에서는 태그 이름(`CardHeader`, `CardContent`)이 역할을 대체하므로 slot 없이도 동일 구조 표현 가능.

**2. 4가지 Card 변형 (별도 컴포넌트)**

| S2 컴포넌트   | 고유 구조                          | XStudio 대응 전략                       |
| ------------- | ---------------------------------- | --------------------------------------- |
...
- `CollectionCardPreview`: 최대 4개 이미지를 1 hero + 3 small grid로 자동 배치. XStudio에서는 CardPreview 내 Image children 개수에 따라 CSS Grid 레이아웃 전환으로 구현.
...
- S2의 slot 기반 Content 패턴은 XStudio의 구조적 자식 모드와 설계 철학이 다름 — 완전한 1:1 매핑 불가
