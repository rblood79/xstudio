...
describe("generateCSS", () => {
  ...
  describe("기본 variant/size", () => {
    it("default variant 배경색 (overlay-background)", () => {
      expect(css).toContain("var(--overlay-background)");
    });

    it("default variant 텍스트색 (text-color)", () => {
  ...
  describe("focusVisible 스타일", () => {
    it("outline 속성 포함", () => {
      expect(css).toContain("outline: 2px solid var(--highlight-background)");
    });
  ...
  describe("size 셀렉터", () => {
    it.each(["xs", "sm", "md", "lg", "xl"])('[data-size="%s"] 존재', (size) => {
...
describe("generateCSS 분기 커버리지", () => {
  ...
  it("textHover 있는 variant → hover에 color 출력", () => {
    const css = generateCSS(
      makeSpec({ textHover: "{color.on-secondary}" as TokenRef }),
    );
  ...
  it("border 있고 borderHover 없는 variant → hover에 backgroundHover로 border-color", () => {
    ...
    expect(css).toContain(
      "border-color: color-mix(in srgb, var(--highlight-background) 85%, black)",
    );
  });
