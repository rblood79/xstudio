...
Proposed — 2026-04-26 (revision 2 — 2026-04-28: pencil app 호환 baseline 확정 + ⑥ Origin 토글 추가 + Properties 패널 ##Component section## 위치 결정 + codex review 7건 반영 (HIGH 3 / MED 3 / LOW 1) + design skeleton 선행 land)
...
| 대안 |             HIGH+             | 판정                                                                                                                                                       |
| ---- | :---------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A    | 1 (R6 조건부 HIGH — codex H3) | **채택 가능 ✅** — G4-G 선결조건 (G4-C + Phase 4-A+B 완료 후) + 0/1/1000 fixture round-trip 으로 mitigation. `instanceCount === 0` 시 LOW silent 경로 보존 |
...
| R6  | ⑥ Origin 토글 (`Cmd+Opt+K`) 의 reusable → standard 해제 시 모든 instance subtree 가 detach materialize 필요 — `instanceCount` 에 비례하는 대규모 변경 위험. 현재 `detachInstance` 는 props 병합만 수행 → nested descendants/path-based override 처리 추가 구현 필요 (codex H3) | **조건부 HIGH** (`instanceCount > 0` 또는 nested descendants 존재 시 HIGH / `instanceCount === 0` 만 LOW silent 가능) | **G4-G 선결조건** = G4-C (detach 본체) + Phase 4-A+B (path-based descendants 처리) 완료 후 진입. 경고 다이얼로그 + ⑤ N개 인스턴스 영향 미리보기 reuse 강제 (`instanceCount > 0`) + undo 정상 + 0/1/1000 instance fixture round-trip + count 정확도 검증 |
...
| **G4-G**: ⑥ Origin 토글 (revision 2 신규)                             | Phase G — **G4-C + Phase 4-A+B 완료 후 진입 (codex H3 선결조건)** | (a) 단축키 `Cmd/Ctrl+Opt/Alt+K` 양방향 토글 (pencil 호환) / (b) Properties 패널 ##Component section## "Create component" 버튼 + `[-]` 토글 / (c) **`instanceCount > 0` 시 G4-E 다이얼로그 reuse 강제 + 경고 (R6 조건부 HIGH 대응)** / (d) **`instanceCount === 0` 일 때만 silent 실행 허용** / (e) reusable → standard 해제 시 모든 instance subtree detach materialize 정합 (50+ fixture: 0/1/1000 instance count round-trip) / (f) undo 정상 | 토글 단축키만 land + Properties 버튼 후순위 + instanceCount 정확도 우선 |
