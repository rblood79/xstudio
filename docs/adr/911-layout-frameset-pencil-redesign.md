...
Frozen — 2026-04-28 (의존 방향 정정 hardening — ADR-912 Component/Slot base 선결)

> **동결 사유** (2026-04-28 세션 49+ / 세션 50+ hardening): 본 ADR 은 ADR-912 (Editing Semantics UI — reusable component + slot 추상) 의 **frame-bundled preset 편의 확장** 임이 framing 재정의 됨. 본 ADR 은 ADR-912 에 영향을 주거나 기준을 제공하지 않는다. `Frame.reusable` / `Frame.slot` / `Ref.descendants` schema 의 사용자 가시 편집 base 는 ADR-912 가 먼저 완료한다. 본 ADR 의 이전 ##Slot section## 소유권 표현은 잘못된 설계 전제였고, ADR-912 기준으로 supersede 된다. **해결**: ADR-912 Component/Slot base 완료 후 본 ADR Phase 3 후속을 재개하며, 그때도 FramesTab / frame preset UX 는 완료된 ADR-912 기능을 더 쉽게 쓰게 하는 보조 흐름만 제공한다.
...
> **재개 조건**: ADR-912 Component/Slot base 완료. 재개 시 P3-ε / P3-ζ 는 ADR-912 기능의 frame authoring 편의 확장으로만 재설계 진입한다.
