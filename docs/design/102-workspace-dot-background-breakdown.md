...
| 경로 | 변경 | LoC 추정 |
| ---- | ---- | -------- |
| `apps/builder/src/builder/workspace/canvas/BuilderCanvas.tsx` | `<canvas>` 직후에 `<DotBackground/>` 삽입 | +2 |
...
export function DotBackground() {
  ...
  // viewport → CSS 변수
  useEffect(() => {
    const apply = (s: { panOffset: { x: number; y: number }; zoom: number }) => {
    ...
    apply(useViewportSyncStore.getState());
    return useViewportSyncStore.subscribe(
      (s) => ({ panOffset: s.panOffset, zoom: s.zoom }),
      apply,
    );
  }, []);
  ...
  return (
    <>
      <div ref={baseRef} className="dot-background dot-background--base" aria-hidden />
      <div
        ref={glowRef}
...
```tsx
// .canvas-container 내부, <canvas> 바로 뒤
<canvas data-testid="skia-canvas-unified" ... />
<DotBackground />   {/* ← 추가 */}
```

DOM 순서는 `<canvas>` → `<DotBackground>` 이지만, z-index 지정(`canvas: 2` / `base: 0` / `glow: 1`)으로 **시각 스택은 배경이 아래**. DOM 의미 순서(주요 콘텐츠가 먼저)와 시각 순서를 모두 만족.
...
- [ ] DevTools Performance: mousemove 중 Skia 재렌더 발생하지 않음
- [ ] DevTools Performance: mousemove frame budget < 2ms
- [ ] 뷰포트 full 영역에서 60fps 유지
...
- [ ] 비교 모드(compareMode) 에서도 양쪽 패널 모두 정상 동작
