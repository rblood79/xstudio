...
- 🎨 **디자인 시스템**: 통합 디자인 토큰 및 테마 관리 (ITCSS 아키텍처)
...
- 🗄️ **[supabase-schema.md](./docs/supabase-schema.md)** - Supabase 스키마

### 개발 가이드
- 🤖 **[CLAUDE.md](./CLAUDE.md)** - AI 코딩 어시스턴트 가이드 (개발 규칙 및 패턴)
...
├── 🏗️ builder/                  # 핵심 빌더 시스템
│   ├── 🤖 ai/                   # AI 어시스턴트
│   ├── 🧩 components/           # React Aria 기반 컴포넌트 (61개)
│   │   └── styles/              # 컴포넌트별 CSS (ITCSS + @layer)
│   ├── 🎣 hooks/                # 빌더 전용 훅
│   ├── 🔍 inspector/            # 속성 편집기 시스템
│   │   ├── components/          # Inspector UI 컴포넌트
│   │   ├── events/              # 이벤트 시스템 (21개 액션 에디터)
│   │   ├── properties/          # 컴포넌트별 속성 에디터
│   │   └── sections/            # 재사용 가능한 섹션
│   ├── 📚 library/              # 컴포넌트 라이브러리
│   ├── 🏠 main/                 # 메인 빌더 컴포넌트
│   ├── 📊 monitor/              # 모니터 시스템 (Memory, Save, History)
│   ├── 🎛️ panels/               # 패널 시스템 (9개 모듈형 패널)
│   │   ├── core/                # PanelRegistry, PanelConfig
│   │   ├── nodes/               # NodesPanel
...
│   ├── 🎨 styles/               # Builder UI 스타일 (ITCSS 구조)
│   │   ├── 1-theme/             # Design tokens (Builder + Preview)
│   │   ├── 2-base/              # Base styles
...
│       ├── PropertiesPanel (속성 편집)
│       ├── StylesPanel (인라인 스타일 관리)
│       ├── DataPanel (데이터 바인딩)
│       ├── EventsPanel (이벤트 핸들러)
│       ├── ThemesPanel (테마 관리)
│       ├── AIPanel (AI 어시스턴트)
│       └── SettingsPanel (프로젝트 설정)
└── BuilderFooter (모니터 시스템)
    ├── Memory Monitor (메모리 추적)
    ├── Save Monitor (저장 성능)
    └── History Monitor (Undo/Redo 추적)
```
...
@layer 계층 구조:
├── dashboard         (대시보드 전용)
├── builder-system    (Builder UI 토큰)
├── preview-system    (Preview 컴포넌트 토큰)
├── shared-tokens     (공통 토큰)
├── components        (React Aria 컴포넌트 61개)
└── utilities         (최고 우선순위)
...
- ✅ 기본 빌더 UI
- ✅ React Aria 컴포넌트 (61개)
- ✅ 실시간 프리뷰 (iframe + computed styles)
- ✅ 인라인 스타일 편집기

**아키텍처**
- ✅ CSS Architecture (ITCSS, @layer 시스템)
- ✅ Panel System (9개 모듈형 패널)
- ✅ Store Modularization (Factory Pattern)
- ✅ Builder/Preview 독립 테마

**고급 기능** (2025-11)
- ✅ Event System (21개 액션 에디터, 비주얼 프로그래밍)
- ✅ Monitor System (Memory, Save, History 추적)
- ✅ Theme System (교차 선택, iframe 통신 개선)
- ✅ Collection Components (DataBinding, removedItemIds)
...
- 🔄 모바일 반응형 편집
- 🔄 컴포넌트 라이브러리 확장
- 🔄 이벤트 시스템 고도화 (조건부 실행, 디버깅)

### v1.2 (계획중)

- 📋 플러그인 시스템
- 📋 외부 API 연동 (REST, GraphQL)
- 📋 고급 애니메이션 지원 (키프레임, 트랜지션)
- 📋 성능 대시보드
- 📋 버전 관리 (Git 통합)
