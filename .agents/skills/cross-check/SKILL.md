...
- CRITICAL 이슈 수정 후 `pnpm run build:specs`와 `pnpm run codex:typecheck`
  필요 여부를 확인한다.
...
발견된 이슈는 즉시 수정합니다. 수정 후 `pnpm run build:specs`와
`pnpm run codex:typecheck` 또는 targeted test로 검증합니다.
