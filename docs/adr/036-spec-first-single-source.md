...
| **composite**        | Card, Calendar, Table, DatePicker             | nested `.child-*` 구조             | **수동 유지** (구조 복잡도)            |
...
| Card        | 34%         | **50%**         | nested .card-\* 구조는 여전히 수동                  |
...
- 자동화율 60–75% 달성 (이전 추정 35–51% 대비 대폭 개선)

단점:

- Archetype 템플릿 개발 투자 (~8개 유형)
- Card, Calendar, Table 등 composite 유형은 여전히 수동 CSS 유지
- 빌드 파이프라인 증가
...
- C: Spec 메타데이터 + Archetype 템플릿으로 CSS 자동 생성 (60–75% 커버)
...
- composite 유형(Card, Calendar, Table)만 수동 CSS 유지 — `validate:sync`로 값 정합 보장

### Rationale

> **핵심 발견: "CSS-only"로 분류했던 패턴 대부분이 이미 Spec shapes()에 데이터가 존재한다.** Switch thumb은 Spec에서 `circle` shape, Checkbox 체크마크는 `line` shape 2개, Radio dot은 `circle` shape으로 정의되어 있다. CSS `::before`는 렌더링 메커니즘이지 데이터가 아니다. 이는 Figma에서 컴포넌트 디자인을 받아 컨버팅할 때와 동일한 관계다 — Figma 레이어의 시각 데이터를 CSS 구조로 매핑하는 것처럼, Spec shapes를 CSS 구조로 매핑할 수 있다.
...
│  ─────────────────────────────────────────          │
│  Card nested .card-* 구조                            │
│  Calendar complex grid                               │
│  Table sticky header + 복합 레이아웃                  │
│                                                     │
│  → ~10개 composite 컴포넌트만                        │
...
| **CSS 자동 생성**    | Level 1 + Level 2      | ~70개 컴포넌트  | 2중→1중 동기화   |
| **값 검증 안전망**   | `validate:sync`        | ~10개 composite | 불일치 자동 감지 |
...
| **XStudio** | **Skia + DOM 이중 출력** | **필요** | **Archetype 템플릿 + shapes 데이터 매핑** |
...
| 컴포넌트              | 이유                                                             |
| --------------------- | ---------------------------------------------------------------- |
| Card                  | nested `.card-type="asset\|user\|product"` 변형이 구조+테마 혼합 |
| Calendar              | complex grid + 날짜 셀 다중 상태 조합                            |
| Table                 | sticky header + 복합 grid/flex + sort indicator                  |
| DatePicker            | Calendar 자식 포함 composite                                     |
| ColorPicker           | gradient canvas + α 슬라이더                                     |
| Menu/Popover          | 위치 계산 + 애니메이션 복합                                      |
| ListBox/GridList/Tree | 가변 길이 목록 + 가상화                                          |
...
4. composite 컴포넌트만 수동 CSS + `validate:sync`
...
| CSS 자동 생성      | 개   | 0             | 0       | 0             | ~20 (등급 A) | ~60 (A+B)           |
| 수동 CSS 유지      | 개   | 88            | 88      | 88            | ~68          | ~**10** (composite) |
...
2. **자동화율 60–75%**: 88개 CSS 중 ~60개를 자동 생성 (이전 추정 35–51% → 대폭 개선)
3. **@media 접근성 패턴 자동화**: 24개 파일의 동일 코드를 템플릿 1개로 대체
4. **Archetype 재사용**: 새 toggle-indicator 류 컴포넌트 추가 시 CSS 자동 생성
5. **validate:sync 안전망**: composite 컴포넌트의 수동 CSS도 값 정합 자동 보장
6. **Figma→XStudio 파이프라인 강화**: Figma 레이어 → Spec shapes → CSS 자동 생성 (ADR-038 연계)

### Negative

1. **Archetype 템플릿 개발 투자**: ~8개 유형 개발 필요
2. **composite 컴포넌트 수동 CSS 유지**: Card, Calendar, Table 등 ~10개는 자동화 불가
