...
### 1.2 Block Formatting Context (BFC) 차이
...
#### 1.3.3 마진 병합 조건 (CSS 명세)

다음 조건을 **모두** 만족해야 병합:
1. 두 마진 모두 block-level 요소
...
다음 중 하나라도 해당하면 병합 안됨:
- `overflow: hidden/auto/scroll` (새 BFC 생성)
- `display: flex/grid/inline-block`
- float 요소
- `position: absolute/fixed`
- 부모에 padding 또는 border 있음
...
**현재 워크어라운드**: BuilderCanvas에서 `flexDirection: 'row'` + `flexWrap: 'wrap'`
...
| Margin collapse (부모-자식) | 미지원 | 지원 | 높음 |
| BFC(Block Formatting Context) | 미지원 | 지원 | 높음 |
