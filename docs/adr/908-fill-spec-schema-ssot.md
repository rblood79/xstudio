...
### ADR-907 과의 관계 (선행/병행 ordering)

[ADR-907 Collection/self-render 컨테이너 style pipeline](907-collection-container-style-pipeline.md) 은 같은 세션에서 Phase 1 Layer A (`packages/specs/src/primitives/cssValueParser.ts`) 를 land 완료 (commit `ff69e4ce` / `a4d1f294`), Layer B (`packages/specs/src/primitives/containerSpacing.ts`) 는 미land 상태다. ADR-908 Phase 1 은 `packages/specs/src/types/spec.types.ts` 에 `FillTokenSpec` (또는 동등) 을 신설한다. 두 ADR 모두 `packages/specs/` 의 primitives/types 공간을 건드리므로 다음 ordering 을 고정한다.

1. **ADR-907 Layer A 는 선행 완료**: cssValueParser SSOT 는 이미 land. ADR-908 Phase 1 은 types/ 공간의 신규 타입 추가이므로 primitives/ 와 독립. 충돌 없음.
2. **ADR-907 Layer B (containerSpacing) 와 ADR-908 Phase 1 은 병행 가능**: Layer B 는 spacing 도메인 (`padding`/`gap`/`borderWidth`), Phase 1 은 fill 색상 도메인. 같은 `primitives/` 디렉터리이나 파일 분리로 merge conflict 없음. 먼저 land 되는 쪽이 후순 ADR 의 primitives/ index 재export 만 조정.
3. **Spec 파일 편집 충돌 영역**: ADR-907 Phase 3 pilot (GridList) 과 Phase 4/5 는 주로 `render.shapes()` / `calculateContentHeight()` 의 spacing 필드를 건드리고, ADR-908 Phase 3 은 `variant.background*` 와 그 소비 체인을 건드린다. 같은 spec 파일에 동시 편집 가능하나 필드 비중첩. Phase 0 inventory 에서 동시 편집 대상 spec 은 병합 정렬 (예: `GridList.spec.ts` 는 ADR-907 Phase 3 이 먼저 land 후 ADR-908 Phase 3 이 진입).
4. **CSSGenerator 측 변경 독립성**: ADR-907 Hard Constraint 8 은 Generator scope 외를 선언. ADR-908 Phase 2 의 Generator bridge 는 ADR-907 과 충돌하지 않으나 동일 파일(`packages/specs/src/renderers/CSSGenerator.ts`) 에 차례로 편집될 가능성이 있어 PR 병합 순서로 관리.
...
3. **incremental migration**: spec 117개 중 `background` 필드를 소비하는 104개 (88%) 를 한 번에 빅뱅 전환하지 않고, resolver/consumer 공통 seam을 먼저 도입한 뒤 단계적으로 이관 가능해야 한다.
4. **visual regression 0**: 기존 spec이 생성하던 CSS/Panel/Skia 기본 appearance의 computed 결과가 대표 컴포넌트 기준으로 회귀 0이어야 한다.
5. **legacy 제거 가능성**: 최종 단계에서 `VariantSpec.background*`, `IndicatorModeSpec.background`, `resolveAppearanceSpecPreset().backgroundColor`의 legacy 해석 경로를 삭제할 수 있어야 한다.
6. **Generator state selector emit 가능성**: CSSGenerator 가 `:hover` / `:pressed` / `[data-selected]` state selector 를 fill preset 소비 시에도 emit 할 수 있어야 한다. ADR-079/080 의 containerStyles direct-read 패턴을 fill preset 에 확장할 때, state-variant 가 CSS 자동 생성 범위 내에 있음을 Phase 1 전제로 확증한다.
...
> 대안 A 채택 결과 **잔존 HIGH 위험 없음**. 위 Risks 는 모두 MED/LOW 로 Gates 의 단계적 게이팅으로 완화 가능.
...
| G4: Legacy Removal      | Phase 4 | `VariantSpec.background*`, `IndicatorModeSpec.background` 및 관련 preset 변환 fallback 삭제 계획과 grep 종료 조건이 충족된다  | legacy bridge를 한 릴리스 더 유지 |
...
- [docs/adr/907-collection-container-style-pipeline.md](907-collection-container-style-pipeline.md)
