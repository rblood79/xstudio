...
- 기존 `TOGGLE_SELECTED_COLORS` 런타임 상수는 Skia `render.shapes` 경로에서 여전히 참조됨 (breaking change 방지) — **후속 과제**: `render.shapes`를 `spec.variants.default.selected*` 직접 참조로 전환 권장

### 잔존 후속 과제

- `TOGGLE_SELECTED_COLORS` Skia 측 참조 → `ToggleButton.spec.variants.default.selected*` 직접 참조 전환 (Skia render.shapes 경로 SSOT 일치)
- `utils/fieldDelegation.ts` 폐지 + ADR-036 "Fully Implemented" 재승격 (B-final)
...
- `TOGGLE_SELECTED_COLORS` 상수: Skia `render.shapes`가 여전히 참조 중. `variants.default.selected*` 필드와 이중 SSOT — 다음 iteration에서 통합 권장.
