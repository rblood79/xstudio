...
- **React 변경 불필요** (**코드베이스 검증 완료**): `packages/shared/src/components/Card.tsx:162`에서 `'data-variant': variant`, `:163`에서 `'data-size': size`를 DOM 속성으로 전달하고 있음. CSS 셀렉터 전환만으로 충분
- `.sm`/`.md`/`.lg` 사이즈도 동일하게 `[data-size]` 셀렉터로 전환
- **추가 확인 필요**: Card.css의 `.selected`, `.quiet` 셀렉터도 data-\* 속성으로 전환 대상 (`data-selected`, `data-quiet`). Card.tsx:166-168에서 해당 속성 전달 여부 구현 시 확인 필요
