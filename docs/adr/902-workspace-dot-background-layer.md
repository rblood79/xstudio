...
Proposed — 2026-04-14 (revised 2026-04-17)
...
4. **pointer-events 무간섭** — 기존 `.canvas-container` DOM 이벤트 체인(ViewportController, 중앙 pointer 핸들러)과 충돌 금지 — ADR-100 Phase 8-9 이후 PixiJS 이벤트 레이어는 제거된 상태
...
- **축 1 대안 A (Skia 내부)**: 성능 HIGH — 커서 이동마다 Skia 전체 재렌더 트리거. ADR-100 Unified Skia 경계("문서 콘텐츠 렌더러") 침범. scene 좌표계 간섭 문제.
...
- Skia 렌더 파이프라인 **렌더 구조는 완전 격리**(clear 상수 1 변경만) → ADR-100 Unified Skia 경계 유지
...
- [ADR-021: Theme System Redesign](completed/021-theme-system-redesign.md)
