...
### 0.1 1차 목표: iframe(Preview)와 동일한 WebGL 렌더링

Builder(WebGL 모드)에서 컴포넌트가 Preview(iframe 모드)와 **시각적으로 동일하게** 그려져야 함.

```
┌─────────────────────────────────────────────────────────────┐
│                    Preview (iframe)                          │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  ElementRenderer → React 컴포넌트 (Button, etc.)    │    │
│  │  → CSS 스타일 직접 적용                             │    │
│  │  → element.props 그대로 전달                        │    │
│  └─────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
                              ↕ 시각적 일치 필요
┌─────────────────────────────────────────────────────────────┐
│                    Builder (WebGL)                           │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  PixiButton, PixiTextField 등 Pixi 컴포넌트        │    │
│  │  → CSS 변수 읽어서 WebGL Graphics로 드로잉          │    │
│  │  → element.props에서 variant, size 추출            │    │
│  └─────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
```

**핵심 검증 항목**:
- [ ] 동일한 variant(primary, secondary, etc.)가 동일한 색상으로 표시
- [ ] 동일한 size(xs, sm, md, lg, xl)가 동일한 크기로 표시
- [ ] 테마 변경 시 양쪽 모두 동일하게 업데이트
- [ ] disabled, hover, pressed 상태 일치

### 0.2 2차 목표: Property/Style 패널 기능 동기화

Property 패널과 Style 패널에서 변경한 값이 WebGL 컴포넌트에 **즉시 반영**되어야 함.

**핵심 검증 항목**:
- [ ] variant 변경 → WebGL 색상 즉시 업데이트
- [ ] size 변경 → WebGL 크기 즉시 업데이트
- [ ] style 속성 변경 → WebGL 스타일 즉시 반영
- [ ] children/text 변경 → WebGL 텍스트 즉시 업데이트
