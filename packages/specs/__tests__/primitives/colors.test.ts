...
describe("Color Tokens", () => {
  ...
  describe("getColorToken", () => {
    it("light 테마에서 올바른 색상 반환", () => {
      expect(getColorToken("primary", "light")).toBe("#2563eb");
    });

    it("dark 테마에서 올바른 색상 반환", () => {
      expect(getColorToken("primary", "dark")).toBe("#3b82f6");
    });

    it("테마 미지정 시 light 기본값", () => {
      expect(getColorToken("primary")).toBe("#2563eb");
    });
  });
