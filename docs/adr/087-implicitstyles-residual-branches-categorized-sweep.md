...
| R4  | synthetic-merge 태그 (SP6 TagGroup/TagList) 는 ADR-072 `_hasChildren` 컨벤션과 상호작용 — Set 재조정 필요 가능 |  MED   | SP6 진입 전 `SYNTHETIC_CHILD_PROP_MERGE_TAGS` Set 감사. 이동 필요 시 ADR-072 Addendum 발행 또는 본 ADR scope 외 분리 |
...
| :--: | ------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------- |
|  G0  | 각 SP 시작          | 진입 criteria (ADR-068/076/079/084/085/086 Implemented + 해당 SP 이전 순서 SP 완료 if dependency 존재) | 선행 SP 또는 ADR 보강                 |
...
|  G4  | 최종 (전체 SP 완료) | implicitStyles 분기 수 40 → 잔존 0 (size-based/business logic 제외한 layout primitive 기준)            | 잔존 분기 list 문서화 + 후속 ADR 배정 |
