...
XStudio는 React Aria Components 기반으로 78개 컴포넌트를 WebGL(Skia) 캔버스 렌더링용으로 구현 완료했다. 그러나 React Spectrum S2에는 React Aria에 없는 **고수준 복합 컴포넌트**가 약 20개 존재하며, 이들은 실제 웹 애플리케이션에서 자주 사용되는 UI 패턴이다.

**현재 상태**:

- React Aria 기반 구현: 78개 (Spec + Factory + Skia 렌더러 + CSS Preview)
- React Spectrum S2 전용: 20개 미구현

### React Aria vs React Spectrum S2 비교

| 계층                  | 역할                                | XStudio 현황    |
| --------------------- | ----------------------------------- | --------------- |
| React Aria Components | 비정형 접근성 프리미티브 (unstyled) | 78개 구현 완료  |
| React Spectrum S2     | Adobe 디자인 시스템 (styled + 복합) | **20개 미구현** |
...
- 기존 78개 컴포넌트와 동일한 편집 경험 (Inspector, Drag, Resize)
...
| G3   | 각 Phase 완료 후 기존 78개 컴포넌트 회귀 없음 | 수동 검증 (주요 컴포넌트 캔버스 배치) |
...
| 문서                   | 경로                                                      |
| ---------------------- | --------------------------------------------------------- |
| React Spectrum S2 스킬 | `.agents/skills/react-spectrum-s2/references/components/` |
| React Aria 스킬        | `.agents/skills/react-aria/references/components/`        |
