...
| --------------------- | ----- | -------- | ------------- | -------------- |
| averageFps            | fps   | _측정_   |               | baseline ± 5%  |
| lastFrameTime         | ms    | _측정_   |               | baseline ± 10% |
| skiaFrameTimeAvgMs    | ms    | _측정_   |               | baseline ± 10% |
| contentRenderTimeMs   | ms    | _측정_   |               | baseline ± 10% |
| blitTimeMs            | ms    | _측정_   |               | baseline ± 20% |
| skiaTreeBuildTimeMs   | ms    | _측정_   |               | baseline ± 15% |
| selectionBuildTimeMs  | ms    | _측정_   |               | baseline ± 15% |
| aiBoundsBuildTimeMs   | ms    | _측정_   |               | baseline ± 15% |
| contentRendersPerSec  | count | _측정_   |               | baseline ± 20% |
| registryChangesPerSec | count | _측정_   |               | baseline ± 20% |
| idleFrameRatio        | ratio | _측정_   |               | baseline ± 10% |
...
`CanvasDebugCategory` enum으로 카테고리별 로깅을 활성화/비활성화한다.
