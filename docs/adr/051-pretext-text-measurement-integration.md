...
**Pretext를 텍스트 줄바꿈/높이 계산의 High-Value Subset SSOT로 도입한다.**

`overflow-wrap: break-word` 텍스트(확정)와 `overflow-wrap: normal` 텍스트(Phase 0 검증 후 결정)에서 CSS Preview와 WebGL/Skia가 **동일한 `prepare()` + `layout()` 결과**를 공유한다. 나머지 CSS 조합(`break-all`, `keep-all`, `anywhere`, `letterSpacing` 등)은 기존 CanvasKit 경로를 유지한다.

**최소 적용 판정선**: Phase 0에서 이중 지표(요소 수 커버리지 ≥ 70% **또는** 핫패스 CPU 기여도 ≥ 50%)를 충족해야 한다. 양쪽 모두 미달이면 도입 효과 부족으로 판단하고 대안 경로(B1~B4)로 전환한다. 상세는 Phase 0 Go/No-Go 이중 지표 참조.
...
- [ ] **Pretext SSOT 실질 적용 비율 측정** — 이중 지표 (아래 상세)

**Go/No-Go 이중 지표** (Codex 토론 반영):

단순 요소 수 비율만으로는 도입 가치를 판단하기 어렵다. 요소 비율이 낮아도 리사이즈 핫패스 CPU 비중이 크거나, CSS↔Skia 시각 차이가 긴 래핑 텍스트에 집중된다면 도입 가치가 있다.

| 지표                  | 통과 기준 | 측정 방법                                                    |
| --------------------- | --------- | ------------------------------------------------------------ |
| **요소 수 커버리지**  | ≥ 70%     | XStudio 실제 프로젝트의 텍스트 요소 중 Pretext 경로 비율     |
| **핫패스 CPU 기여도** | ≥ 50%     | Step 4.5 re-enrich 시간 중 Pretext 경로 요소가 차지하는 비율 |

- 두 지표 **모두** 통과 → Phase A 진행
- 하나만 통과 → 예외 승인 검토 (핫패스 기여도 우선)
- 모두 미달 → Phase 0 실패 → 대안 경로 전환

**Phase 0 실패 시 대안 경로** (Codex 토론 반영):

| 대안   | 내용                                                                    | 기대 효과                           | 비용  |
| ------ | ----------------------------------------------------------------------- | ----------------------------------- | ----- |
| **B1** | `cssNormalBreakProcess()`를 공식 compatibility layer로 승격/정리        | 현재 보정 해킹 체계화               | 1~2일 |
| **B2** | `measureNaturalWidth`만 Pretext 사용, `measureWrapped`는 CanvasKit 유지 | fit-content 폭 개선, 높이 기존 유지 | 2~3일 |
| **B3** | CanvasKit 차기 버전 Paragraph API 변경 조사 태스크 분리                 | 장기 개선 가능성 파악               | 0.5일 |
| **B4** | 외부 라이브러리 리서치 조건부 개시 (No-Go 판정 후 2주 이내)             | 대안 후보 발굴                      | 1~2일 |

**B2는 Phase 0 부분 실패 시 유효**: Pretext `measureNaturalWidth()`는 CSS 동일 엔진 기반이므로, fit-content/max-content 폭 계산에서 현재 `canvas2dCompensation` +2/+4px 해킹을 대체할 수 있다. 줄바꿈/높이 정합성은 포기하지만, 폭 정합성 개선만으로도 가치가 있다.
...
`@chenglou/pretext`를 **dependency**(런타임 사용)로 추가하고, 핵심 API를 래핑하는 어댑터를 작성한다. 또는 소스 vendor(`packages/shared/vendor/pretext/`) 후 직접 import — 성숙도 Risk 4 완화를 위해 vendor를 권장한다.
...
class PretextMeasurer implements TextMeasurer {
  ...
  // ── Pretext capability flags (Phase 0 검증 결과로 확정) ──
  // Phase 0 완료 후 검증 결과에 따라 true/false로 고정한다.
  private static readonly SUPPORTS_OVERFLOW_WRAP_NORMAL = false; // Phase 0 검증 대기
  ...
  private needsFallback(style: TextMeasureStyle): boolean {
    ...
    if (style.wordBreak === "break-all" || style.wordBreak === "keep-all")
      return true;
    // overflow-wrap: normal → capability flag로 제어
    if (
      !PretextMeasurer.SUPPORTS_OVERFLOW_WRAP_NORMAL &&
      (!style.overflowWrap || style.overflowWrap === "normal")
    )
      return true;
    return false;
  }
...
| ----- | ----------------------------------------- | --------------------------------------- | --------------------------------------------- |
| **0** | —                                         | —                                       | POC 코드 삭제                                 |
| **A** | Phase 0 Go/No-Go 통과                     | `USE_PRETEXT_MEASURER` (dev 환경 변수)  | `setTextMeasurer(canvasKitMeasurer)` 1줄 변경 |
| **B** | Phase A 단위 테스트 통과 + A/B 검증 ≤ 2px | 동일 플래그 (A와 공유)                  | measurer 교체로 즉시 rollback                 |
| **C** | Phase B 벤치마크 통과                     | `USE_PRETEXT_BREAK_HINTS` (별도 플래그) | Break Hint 분기 `if` 제거 → 기존 경로 복원    |
| **D** | Phase C 시각 비교 통과                    | Phase C 플래그에 의존                   | min-height 힌트 제거 / overlay 크기 원복      |
...
**E2E 측정 전제 조건** (수치 재현/비교 시 필수 기록):

- 텍스트 요소 수: 500개 (1차), 100개 (Step 4.5 불일치 노드)
- PreparedText 캐시 상태: warm (prepare() 사전 완료, 폰트 변경 없음)
- 폰트: Inter 14px, Pretendard fallback
- Taffy 호출: `computeLayout()` 1회 재호출
- 브라우저: Chrome stable, macOS (devicePixelRatio=2)
- 측정 방법: `performance.now()` 10회 반복 중앙값
...
#### C-2d. Break Hint Paragraph 캐시 키 규칙 (Codex 2차 지적)

Break Hint 경로에서 `hintedText`(\n 삽입)가 렌더링 텍스트가 되므로, Paragraph 캐시 키가 올바르게 구성되어야 한다.
...
| `layout()` 결과    | `(PreparedText handle, maxWidth, lineHeight)`              | maxWidth 변경 시    |
| `Paragraph` (렌더) | `(hintedText, maxWidth, fontFamilies, fontSize, ...style)` | hintedText 변경 시  |

**핵심**: `maxWidth`가 변경되면 Pretext 줄바꿈이 달라지고 → `hintedText`도 달라지므로 → Paragraph 캐시가 자동 무효화된다. 기존 nodeRendererText.ts의 캐시 키(L114-136)에서 `processedText` 필드가 이미 이 역할을 하므로, Break Hint 경로에서도 `hintedText`를 `processedText`로 전달하면 기존 캐시 메커니즘과 호환된다.

**원문-hintedText offset mapping**: Break Hint에서 삽입된 `\n`의 위치를 원문 오프셋으로 역매핑하는 테이블은 현재 불필요 (텍스트 선택/커서 위치는 Quill DOM이 담당). Phase D-2에서 Quill 오버레이와 Skia 텍스트 간 커서 동기화가 필요해지면 offset mapping 도입을 검토한다.
...
> **크기 척도 참고**: "소스"는 TypeScript 소스 코드 크기, "minified"는 npm 배포 번들, "런타임"은 브라우저 메모리 사용량.

| 항목                    | 현재                | Pretext 도입 후                             |
| ----------------------- | ------------------- | ------------------------------------------- |
| CanvasKit WASM          | ~6MB                | ~6MB (렌더링용 유지)                        |
| Pretext 라이브러리      | 0                   | ~15KB minified (런타임 파싱 후 ~50KB 힙)    |
| Paragraph 객체 (측정용) | ~500KB (LRU 1000개) | 0 (측정용 Paragraph 불필요)                 |
| PreparedText 캐시       | 0                   | ~200KB (SoA 구조, 500 텍스트)               |
| **순 변화**             | —                   | **~-250KB** (Paragraph 캐시 축소 > Pretext) |
...
**심각도**: **MEDIUM** — 대부분의 UI 텍스트에 무영향, 커스텀 spacing 텍스트만 fallback. Canvas `measureText()` 자체의 한계이므로 Pretext 측에서 해결 불가 ([#78](https://github.com/chenglou/pretext/issues/78))
...
**`lineCount` 추가의 호출자 영향 범위**:

`TextMeasureResult.lineCount`는 optional(`lineCount?: number`)이므로 기존 호출자는 수정 불필요. 새 호출자만 opt-in으로 사용:

- `enrichWithIntrinsicSize` (utils.ts) — 현재 height만 사용, lineCount는 Phase B에서 선택적 활용
- `specShapeConverter.ts` — Phase C에서 TextShape에 lineCount 전달
- `Canvas2DTextMeasurer` — 내부적으로 lineCount를 계산하나 반환하지 않았음 → 반환 추가 (파손 없음)
- `CanvasKitTextMeasurer` — `paragraph.getLineMetrics().length`로 lineCount 반환 추가
...
### Phase 0 (Feasibility POC) — Canonical Go/No-Go 게이트

> **참고**: 아래 10개 항목이 Phase 0의 **유일한 정의**이다. 본문 섹션 0-1~0-7은 각 항목의 상세 설명이며, 이 체크리스트와 1:1 대응한다.

- [ ] **0-1** Canvas 2D context + WebGL context 공존 확인 (Pretext가 별도 canvas 생성 검증)
- [ ] **0-2** 폰트 로딩 타이밍 차이 측정 + 이중 감지 패턴 작동 확인
- [ ] **0-3** 한국어+영어 혼합 텍스트에서 Pretext width vs CSS width 오차 ≤ 1px
- [ ] **0-4** word-break/overflow-wrap 커버리지 매트릭스 확정 (Pretext 소스 검증)
- [ ] **0-5** `overflow-wrap: normal` 시맨틱 검증 — Pretext가 단어 미분할 모드를 지원하는지 확인
- [ ] **0-6** CJK+숫자/URL 혼합 줄바꿈 검증 ([#96](https://github.com/chenglou/pretext/issues/96) 패턴)
- [ ] **0-7** Break Hint 텍스트 원본 보존 검증 (trailing space, 연속 공백, 탭)
- [ ] 시스템 폰트 vs CDN 폰트 차이 — `document.fonts.check()` true이나 바이너리 다른 경우 측정
- [ ] Pretext SSOT 실질 적용 비율 측정 — **70% 미만이면 도입 효과 부족으로 재검토**
- [ ] 500 요소 기준 prepare() 벤치마크 (19ms 이내)
- [ ] **Go/No-Go 판정**: 위 항목 전부 통과 + 적용 비율 ≥ 70% 시 Phase A 진행, 실패 시 대안 검토
...
## 외부 근거 vs 제품 내부 검증 필요 항목

### 외부에서 확인된 사실 (커뮤니티 리서치 2026-04-04)

| 항목                                  | 근거                                                                                                                |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Pretext `layout()` 0.0002ms/text      | 저자 벤치마크 + 커뮤니티 재현 (Typexperiments, Textura)                                                             |
| `overflow-wrap: break-word` 하드코딩  | Pretext 소스 `line-break.ts` 확인 + `PrepareOptions`에 overflowWrap 미포함                                          |
| 아랍어 혼합 최대 78.6px 불일치        | [#77](https://github.com/chenglou/pretext/issues/77) 재현 가능 벤치마크                                             |
| keep-all 미지원                       | [#74](https://github.com/chenglou/pretext/issues/74) + PR [#83](https://github.com/chenglou/pretext/pull/83) 미병합 |
| letterSpacing Canvas measureText 무시 | [#78](https://github.com/chenglou/pretext/issues/78) + Web API 스펙 제약                                            |
| CSS 정합성 4건 불일치 (Inter 16px)    | [#89](https://github.com/chenglou/pretext/issues/89) Chrome 146 기준                                                |
| Canvas 2D ↔ WebGL 공존 가능           | Pretext 소스 `measurement.ts` — 별도 canvas element 생성                                                            |

### XStudio 환경에서 Phase 0 검증 필요 가설

| 항목                               | 가설                                                                                         | 실패 시 영향                 |
| ---------------------------------- | -------------------------------------------------------------------------------------------- | ---------------------------- |
| `normal+normal` Pretext 적용 가능  | Pretext가 overflow 허용 모드를 지원할 수 있음                                                | SSOT 범위 축소 (1개 조합만)  |
| 한국어+숫자 혼합 줄바꿈 정확       | [#96](https://github.com/chenglou/pretext/issues/96) 패턴이 한국어에서 재현되지 않을 수 있음 | 해당 패턴 CanvasKit fallback |
| Break Hint `join("\n")` 원본 보존  | Pretext가 공백 정규화하지 않을 수 있음                                                       | Break Hint 전략 재설계 필요  |
| Pretext SSOT 적용 비율 ≥ 70%       | XStudio 텍스트의 대부분이 `break-word` 경로                                                  | 도입 효과 부족 시 재검토     |
| E2E 6.5~10× 성능 개선              | warm 캐시 + Inter 14px 전제 하에 달성 가능                                                   | 개선폭 하락 시 ROI 재평가    |
| CDN 폰트 vs 시스템 폰트 차이 ≤ 1px | Google Fonts CDN과 로컬 설치 폰트 바이너리 동일                                              | check() 가드 강화 필요       |
