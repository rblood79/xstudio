...
**Deferred** — 2026-04-04 (v3: 조건부 재평가 — ROI 분석 결과 현시점 도입 보류)
...
| ----------- | --------------------------------------------------------- |
| 구현        | Phase A~D, 2~3주                                          |
| 복잡도 증가 | TextLayoutService + PretextMeasurer + 이중 코드 경로 유지 |
| 번들        | +15KB                                                     |
| vendor 관리 | commit SHA 고정, PR #82/#83 병합 대기, 이슈 추적          |
| 테스트      | break-word(Pretext) + normal(CanvasKit) 양 경로 검증 필수 |

#### 도입 효과 (v3: 기본값 `normal` 유지)

| 항목                     | 효과                                                       |
| ------------------------ | ---------------------------------------------------------- |
| SSOT 적용 범위           | ~29% (break-word 프리셋 선택 요소만) → PR #83 병합 시 ~43% |
| break-word 경로 정합성   | CSS↔Skia ~99% (해당 요소만)                                |
| break-word 경로 성능     | 리사이즈 6.5×, 2-Pass 가속                                 |
| **normal 경로 (기본값)** | **변화 없음** — 대부분의 텍스트(~70-80%)가 이 경로         |
| 디자인 품질              | 보존 (기본값 변경 없음)                                    |

#### Pretext 라이브러리 리스크 현황

| 이슈/PR | 내용                                            | 상태                       | 영향         |
| ------- | ----------------------------------------------- | -------------------------- | ------------ |
| #89     | CSS↔Pretext 불일치 4건 (영문, URL, CJK, 이모지) | **Open, 미해결**           | **HIGH**     |
| #77     | 아랍어 BiDi 최대 78.6px 차이                    | Open, PR #82가 수정        | HIGH→MEDIUM  |
| #96     | 중문+연속숫자 줄바꿈 오류                       | **Open, 미해결**           | MEDIUM       |
| #98     | "$\_\_\_" 패턴 줄바꿈 불일치 (신규)             | **Open, 미해결**           | MEDIUM       |
| #78     | letterSpacing 미지원                            | Open, 제안만               | MEDIUM       |
| #83     | keep-all 지원 PR                                | Open, 리뷰 준비 완료       | 병합 시 해소 |
| #82     | BiDi 수정 PR                                    | Open, 리뷰 준비            | 병합 시 해소 |
| 버전    | v0.0.4 (pre-1.0), API 변경 이력 있음            | `measureNaturalWidth` 제거 | 안정성 우려  |

#### 판정: **Deferred (현시점 도입 보류)**

| 판정 근거                                                                    |
| ---------------------------------------------------------------------------- |
| v2의 핵심 전제(`break-word` 기본값)가 디자인 품질 문제로 기각 → ROI 80% 소실 |
| 기본값 `normal`인 대부분의 텍스트(~70-80%)가 혜택 없음                       |
| 이중 코드 경로(Pretext + CanvasKit) 유지 비용 vs ~29% SSOT 효과 불균형       |
| #89 기본 영문에서도 CSS 정합성 실패 — break-word 경로마저 신뢰 불확실        |
| v0.0.4 pre-1.0, 핵심 PR (#82/#83) 미병합, 66개 open issues                   |

#### 재평가 트리거

| 조건                                                       | 효과                             |
| ---------------------------------------------------------- | -------------------------------- |
| `PrepareOptions`에 `overflowWrap` 옵션 추가                | 기본값 변경 없이 SSOT ~100% 가능 |
| Pretext 1.0 릴리스                                         | API 안정성, 품질 보증            |
| PR #82 + #83 + #89 해결 모두 병합                          | 핵심 리스크 해소                 |
| XStudio 사용자 피드백에서 break-word 기본값 수용 가능 확인 | v2 전략 재개                     |

#### 현시점 대안

| 대안                     | 비용  | 효과                                          |
| ------------------------ | ----- | --------------------------------------------- |
| CanvasKit 경로 최적화    | 1~2일 | `cssNormalBreakProcess` 정리, LRU 히트율 향상 |
| Pretext 모니터링         | 0     | 재평가 트리거 조건 추적                       |
| Phase 0 검증만 사전 실행 | 0.5일 | 트리거 충족 시 즉시 착수 가능 (데이터 확보)   |
