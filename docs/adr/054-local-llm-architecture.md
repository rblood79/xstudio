...
React Aria Components / React Spectrum S2 문서를 기반으로, 자연어 명령만으로 컴포넌트의 모든 props, variants, 상태, 접근성 패턴을 자유롭게 활용할 수 있어야 한다.
...
- 리소스: React Aria 52개 컴포넌트 문서(~216K tok), S2 71개 문서(~95K tok), XStudio Spec 65+ 컴포넌트
...
| G4: 컴포넌트 지능 Props 정확도 | Phase 5 완료 | 컴포넌트 카탈로그 기반 props 설정 성공률 ≥ 90% (유효하지 않은 prop 조합 ≤ 10%) | 카탈로그 형식 재설계, 문서 로딩 전략 변경                          |
| G5: 디자인 지능 복합 레이아웃  | Phase 6 완료 | "대시보드" 수준 요청 시 올바른 flex/grid + 3개 이상 컴포넌트 배치 성공         | 레이아웃 템플릿 확대, Plan 단계 프롬프트 튜닝                      |
...
- Plan→Execute→Verify 에이전트 루프로 "대시보드 만들어줘" 수준의 복합 요청 처리 가능
