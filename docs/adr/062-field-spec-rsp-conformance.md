...
1. **RSP = 최우선 권위** — 단순 레퍼런스가 아니라 Adobe가 수년간 접근성·API 일관성·UX 패턴을 검증한 설계 결정체. RSP가 규정한 Field API 형태(예: `variant` 부재, `isQuiet` 존재, `isInvalid` 상태 기반 오류 표현)는 **Spec이 뒤집을 권한 없음**.
2. **Spec = RSP 미규정 영역의 기준** — RSP가 명시하지 않은 영역(Canvas/Skia 렌더 토큰, typography/color/레이아웃 세부 값, 프로젝트 고유 확장)에서만 Spec이 결정권자. 이 영역에서는 Spec=SSOT.
3. **CSS ≡ Skia = symmetric consumers** — 양쪽 모두 Spec에서 파생, 대칭 회귀(`/cross-check`)로 검증.

따라서 결정 주어는 **"RSP가 결정한 영역에서는 RSP를 따르고, 그 외에서는 Spec이 결정한다"**. 현재 Field의 `variant` prop은 RSP 규정 영역(Field API 형태)에 대한 이탈 → 본 ADR은 **RSP 결정 복원**이지 Spec 내부 리팩토링이 아님.

**역사적 연속성 — 본 원칙은 신규 도입이 아니라 명문화**:

composition은 RSP/React Aria 우선 원칙을 이미 암묵적으로 적용해 왔다:

- **ADR-058 Phase 5 Deferred 결정 (2026-04-11)**: rendererMap이 `spec.element` 미사용 — 하드코딩 or "**React Aria 기본값**"을 사용. "엄격한 Spec-First SSOT"보다 "React Aria 구조 수용"을 선택. 이는 Spec을 100% 구현하지 않고 RSP/React Aria의 검증된 구조를 존중한 결정 → **RSP 우선 원칙의 실천**.
- **ADR-057 기존 구현 완전 재사용 (2026-04-11)**: ADR-005의 Skia 렌더러와 textWrapUtils를 Spec 중심으로 재작성하지 않고 그대로 재사용 — "외부 검증 설계 존중" 계통.

따라서 본 ADR의 "RSP 최우선 + Spec 보완" 원칙은 **새 규칙이 아니라 기존 운영 실태의 명시적 문서화**. 이로써 향후 ADR(예: 현재 일시 중단 중인 ADR-059 v2)에서 Spec vs RSP 판단 기준이 명확해진다.
...
- 근거: Spec=SSOT 원칙 하에서 RSP는 참조일 뿐 규범이 아님. 프로젝트 디자인 의도(per-instance 색상 variation)를 포기하지 않고 유지. 기존 34파일 변경을 피함.
- 위험:
...
3. **SSOT 체인 정합성** — HIGH 수용이 "RSP 최우선 + Spec 보완" 원칙과 충돌하지 않는가?
...
1. **RSP 결정 영역 복원 (first principle)** — SSOT 체인 #1에 따라 RSP가 규정한 Field API 형태를 따름. A만 유일하게 이 원칙에 정합 (B는 deprecated 경로 유지로 이탈 지속, C는 variant 유지로 영구 이탈).
...
- **대안 B 기각**: 유지보수 HIGH(deprecated 코드 장기 잔존)는 SSOT 원칙을 장기 훼손하며 ADR-036 재승격 차단. 사용자 전환 완화 이득은 composition 초기 단계에서 weight 낮음.
- **대안 C 기각**:
  1. **원칙 위반 (first principle)** — SSOT 체인 #1(RSP 최우선)에 따라 RSP가 규정한 Field API 형태(variant 부재, isQuiet/isInvalid 기반)는 Spec이 뒤집을 권한 없음. "variant 유지 + 비표준 확장 문서화"는 RSP 결정 영역 침범. Spec=SSOT는 RSP 미규정 영역에 한정 적용되며, Field의 variant는 RSP 규정 영역에 속함.
