...
| Phase 3 완료 | Phase 2 후 | `showValue` 3컴포넌트에서 제거. Slider는 SliderOutput 자식 패턴. autoCorrect enum 전환. 6개+ 참조 경로 동작 확인 | Phase 2까지만 유지                      |
...
- S2에 없는 `showValue` prop 제거 → S2 API와 정확히 일치
- NumberField 이중 계약(개별 props + formatOptions) 정리 → 단일 소스
- 총 props 감소: Phase 1(-0) + Phase 2(-7) + Phase 3(-3 showValue 제거, -0 autoCorrect 타입만) = **순 -10 props**
...
- Phase 3 Slider `showValue` 제거 시 기존 showValue=false 데이터는 SliderOutput 자식 제거로 마이그레이션 필요
