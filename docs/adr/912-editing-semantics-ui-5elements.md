...
1. **5 요소 land 필수** (ADR-903 Hard Constraint #10):
   - ① reusable / ref / override 시각 마커 2종 (LayerTree + Canvas) — DesignKit 마커는 [ADR-915](completed/915-remove-designkit-system.md) 로 제거됨
   - ② 원본 ↔ 인스턴스 양방향 탐색 액션
...
5. **Skia 정적 렌더 제약** — Builder Skia 는 default state 만 정적 렌더한다. button 의 hover / focus / press 같은 runtime interactive state 는 Preview/Publish DOM 단독 영역 (ssot-hierarchy.md D3 대칭 = default state 한정). 따라서 마커 vs button runtime state 충돌 케이스 **부재**. 마커 충돌 검토 범위 = **Skia 편집기 cue 2종** (선택 blue solid / hover blue dashed alpha 0.5) + **LayerTree cue 3종** (selected outline / hover outline / focused 2px outline)
6. **pencil app 시각 언어 정합 검토 필수** — composition 은 ADR-911/913/914 에서 pencil.dev `.pen` schema 호환을 명시 채택. pencil app 자체는 component origin = **magenta** bounding box / component instance = **violet** bounding box 사용 (선택 시). 마커 색상 결정 시 pencil 정합 (외부 호환 우선) vs composition 토큰 정합 (D3 SSOT 우선) 결정 Phase A 진입 전 lock. override descendants 마커는 pencil 공식 docs 직접 접근 차단 (403) — Phase A 진입 전 pencil app 실측 또는 composition 자체 결정 필요
...
| ID  | 위험                                                                                                                                                                                                                              | 심각도 | 대응                                                                                                                                                                                                                                                           |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | 시각 마커 3종이 **Skia 편집기 cue 2종** (선택 blue solid / hover blue dashed alpha 0.5) + **LayerTree cue 3종** (selected/hover/focused outline) 과 충돌. button runtime state 는 Skia 미렌더 → 충돌 범위 외 (Hard Constraint #5) |  MED   | Phase A 진입 전 (a) pencil app 실측 — origin=magenta / instance=violet 정합 vs composition 토큰 결정 / (b) Skia 색상 채널 분리 검증 (blue ≠ marker 색) / (c) LayerTree 마커 위치 분리 (row trailing icon = Trash 옆) / (d) Storybook 3 cue 동시 발생 시각 검증 |
...
| ------------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| **G4-A**: 시각 마커 2종 land          | Phase A | (a) LayerTree/Canvas 에서 reusable/ref/override 각각 고유 시각 마커 / (b) pencil app 실측 + 색상 정합 결정 lock (origin=magenta / instance=violet vs composition 토큰) / (c) Skia 편집기 cue 2종 (선택 blue / hover blue dashed) + LayerTree cue 3종 (selected/hover/focused) 과 충돌 0 — button runtime state 는 Skia 미렌더 (Hard Constraint #5) → 충돌 범위 외 / (d) Storybook 3 cue 동시 발생 식별 100% | 디자인 재검토 + low-friction 마커 fallback   |
...
- pencil app schema (`.pen` format) — 본 ADR 의 ref/override 의미 호환 기준
- pencil app 시각 마커 — component origin = magenta bounding box / component instance = violet bounding box (선택 시). 출처: [docs.pencil.dev/core-concepts/components](https://docs.pencil.dev/core-concepts/components) (직접 fetch 403, 검색 인용)
- [ssot-hierarchy.md](../../.claude/rules/ssot-hierarchy.md) D3 — Skia 정적 렌더 제약 (button runtime state = DOM 단독, default state = 대칭 검증 대상)
