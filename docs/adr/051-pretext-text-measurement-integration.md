...
Proposed — 2026-04-03 (2026-04-04 v2: overflow-wrap:break-word 기본값 전제 재작성)
...
### 해결: `overflow-wrap: break-word` 기본값 전환

**Phase 0 결과의 핵심 인사이트**: Pretext의 "한계"가 아니라 XStudio의 **기본값 선택**이 불일치의 원인이다.

`overflow-wrap: break-word`는 CSS에서 **더 방어적인 기본값**이다:

| 측면            | `normal` (현재)                      | `break-word` (변경 후)       |
| --------------- | ------------------------------------ | ---------------------------- |
| 긴 단어(URL 등) | 컨테이너 밖으로 오버플로             | 줄바꿈으로 컨테이너 내 유지  |
| 일반 UI 텍스트  | 차이 없음 (단어가 컨테이너보다 짧음) | 차이 없음                    |
| Pretext 호환    | ❌ 시맨틱 불일치                     | ✅ 완전 일치                 |
| SSOT 커버리지   | ~29%                                 | **~100%** (normal wordBreak) |
| 업계 관례       | —                                    | Tailwind `break-words` 기본  |

**변경 포인트**: `cssResolver.ts` L83 (`CSS_INITIAL_VALUES`) + L286 (`ROOT_COMPUTED_STYLE`) — 2곳 동시 변경

이 2줄 변경으로 Pretext SSOT 전면 도입이 가능해진다.
...
**`overflow-wrap: break-word`를 XStudio 기본값으로 전환하고, Pretext를 텍스트 줄바꿈/높이 계산의 SSOT로 도입한다.**

CSS Preview와 WebGL/Skia가 동일한 `prepare()` + `layout()` 결과를 공유하여, 브라우저 네이티브 Canvas `measureText()`를 ground truth로 통일한다. `break-all`, `keep-all`, `letterSpacing` 등 Pretext 미지원 조합은 CanvasKit fallback을 유지한다.
...
1. **Pretext = CSS `break-word` 재현** — XStudio 기본값을 `break-word`로 맞추면 Pretext와 CSS가 동일 시맨틱
2. **Canvas measureText() 기반** — 브라우저 네이티브 폰트 엔진을 ground truth로 사용
3. **2-Phase 분리** — `prepare()`(비싼) + `layout()`(0.0002ms) 분리로 리사이즈 핫 패스 최적화
...
### 전제 조건: `overflow-wrap: break-word` 기본값 전환 (Phase 0)

```typescript
// cssResolver.ts — 2곳 동시 변경 필수
// L83 (CSS_INITIAL_VALUES)
overflowWrap: "break-word", // 이전: "normal"
// L286 (ROOT_COMPUTED_STYLE)
overflowWrap: "break-word", // 이전: "normal"
```

**영향 범위 분석**:

- **일반 UI 텍스트** (Button, Label, Badge): 단어가 컨테이너보다 짧음 → **차이 없음**
- **긴 URL/경로**: `overflow: hidden` 또는 `text-overflow: ellipsis`와 조합 시 → **차이 없음** (이미 잘림)
- **자유 텍스트**: 긴 단어가 줄바꿈됨 → **더 나은 UX** (오버플로 방지)
- **CSS Preview**: 브라우저가 `break-word`를 네이티브 처리 → **정합성 향상**
- **CanvasKit**: 기존 `preprocessBreakWordText()` 경로 활용 → **기존 동작 유지**

**TypographySection 프리셋 변경** (Phase 0 동시 적용):

"Normal" 프리셋을 **제거**하고 "Break Words"를 기본 프리셋으로 승격한다. 기본값이 `break-word`이므로 별도 "Normal" 옵션은 혼란을 야기한다.

| 프리셋          | `overflowWrap` 설정 | 동작                                          |
| --------------- | ------------------- | --------------------------------------------- |
| **break-words** | `"break-word"`      | **기본값** — Pretext SSOT 경로                |
| nowrap          | `""`                | 줄바꿈 금지 (`whiteSpace: "nowrap"` 지배)     |
| truncate        | `""`                | 말줄임 (`whiteSpace: "nowrap"` + ellipsis)    |
| break-all       | `""`                | 문자 단위 분할 (`wordBreak: "break-all"`)     |
| keep-all        | `"break-word"`      | CJK 단어 보존 (`wordBreak: "keep-all"`)       |
| preserve        | `""`                | 공백 보존 (`whiteSpace: "pre-wrap"`)          |
| custom          | (사용자 지정)       | 개별 속성 직접 설정 (overflow-wrap:normal 등) |

> `deriveTextBehaviorPreset()`에서도 `'normal'` 반환을 제거 — 기존 프로젝트에서 `overflowWrap: "normal"`이 명시된 경우 "Custom..."으로 표시된다.
...
| Phase | 진입 조건                            | 배포 플래그                             | 되돌림 단위                                   |
| ----- | ------------------------------------ | --------------------------------------- | --------------------------------------------- |
| **0** | —                                    | —                                       | `cssResolver.ts` L83+L286 원복 (2곳)          |
...
### Phase 0: 기본값 전환 + 환경 검증 (1~2일)

#### 0-0. `overflow-wrap: break-word` 기본값 전환

```typescript
// cssResolver.ts L83 (CSS_INITIAL_VALUES) + L286 (ROOT_COMPUTED_STYLE) — 2곳 동시 변경
overflowWrap: "break-word", // ← 변경 (이전: "normal")
```

**검증**: CSS Preview + Canvas 전체 회귀 테스트. 기존 프로젝트에서 시각적 차이가 발생하는 요소를 식별하고, 대부분이 "오버플로 방지"로 개선되는지 확인.
...
- [ ] `overflow-wrap: break-word` 기본값 전환 후 회귀 없음
- [ ] Canvas 2D + WebGL 공존 확인
- [ ] 폰트 로딩 이중 감지 작동
- [ ] 한국어+영어 혼합 Pretext width vs CSS width 오차 ≤ 1px
- [ ] CJK+숫자 혼합 줄바꿈 정확
...
| 대안   | 내용                                                              | 비용   |
| ------ | ----------------------------------------------------------------- | ------ |
| **B1** | `cssNormalBreakProcess()` 정리/최적화 — CanvasKit 보정 체계화     | 1~2일  |
| **B2** | `layoutWithLines().lines[].width`로 fit-content 폭만 Pretext 사용 | 2~3일  |
| **B3** | CanvasKit 차기 버전 Paragraph API 변경 조사                       | 0.5일  |
| **B4** | Pretext에 overflowWrap 옵션 PR 기여 → 병합 후 재평가              | 불확실 |
...
class PretextMeasurer implements TextMeasurer {
  ...
  private needsFallback(style: TextMeasureStyle): boolean {
    ...
    if (style.wordBreak === "break-all" || style.wordBreak === "keep-all")
      return true;
    // overflow-wrap: normal은 Pretext 미지원이나, 기본값을 break-word로 전환했으므로
    // normal이 명시적으로 설정된 경우만 fallback
    if (style.overflowWrap === "normal") return true;
    return false;
  }
}
```
...
**height-only 최적화 경로**: width가 불필요한 호출(Step 4.5 re-enrich)에서는 `layout()` fast path (0.0002ms)를 직접 사용:
...
| 조합                      | 기본값 전환 후 | Pretext 도입 후                                                                           | 비고                 |
| ------------------------- | -------------- | ----------------------------------------------------------------------------------------- | -------------------- |
| `normal` + `break-word`   | **기본값**     | **Pretext Break Hint** ✅                                                                 | 핵심 SSOT 경로       |
| `normal` + `normal`       | 명시 설정만    | **CanvasKit 유지**                                                                        | 사용자가 명시적 선택 |
| `normal` + `anywhere`     | 명시 설정만    | **CanvasKit 유지**                                                                        | Pretext 미지원       |
| `break-all` + (any)       | 명시 설정만    | **CanvasKit 유지**                                                                        | 문자 단위 분할       |
| `keep-all` + `break-word` | 프리셋 선택    | **CanvasKit 유지** (PR [#83](https://github.com/chenglou/pretext/pull/83) 병합 후 재평가) | 한국어 단어 보존     |

**핵심**: 기본값을 `break-word`로 전환했으므로, 대부분의 텍스트가 Pretext 경로를 타고, 나머지 5개 조합만 CanvasKit fallback.
...
| 항목                             | 가설                                | 실패 시 영향           |
| -------------------------------- | ----------------------------------- | ---------------------- |
| break-word 기본값 전환 회귀 없음 | 일반 UI 텍스트에서 시각적 차이 없음 | Phase 0 중단           |
| CJK+숫자 줄바꿈 정확             | #96 패턴이 한국어에서 재현 안 됨    | 해당 패턴 fallback     |
| Break Hint 텍스트 보존           | 공백 정규화가 줄바꿈에 영향 안 줌   | Break Hint 전략 재설계 |
| E2E 6.5~10× 성능 개선            | warm 캐시 전제 하에 달성            | ROI 재평가             |
...
| 파일                                 | 변경 내용                                                                                  | Phase |
| ------------------------------------ | ------------------------------------------------------------------------------------------ | ----- |
| `cssResolver.ts`                     | L83 + L286 `overflowWrap: "normal"` → `"break-word"` (2곳)                                 | 0     |
| `TypographySection.tsx`              | "Normal" 프리셋 제거, "Break Words"를 기본으로 승격                                        | 0     |
| `useTypographyValuesJotai.ts`        | `deriveTextBehaviorPreset()`에서 `'normal'` 반환 제거                                      | 0     |
...
| 코드                        | 제거 조건                                             |
| --------------------------- | ----------------------------------------------------- |
| `preprocessBreakWordText()` | **확정 제거** — Pretext가 break-word 네이티브 처리    |
| `cssNormalBreakProcess()`   | **보류** — overflowWrap:normal fallback에 여전히 필요 |
...
- [ ] `overflow-wrap: break-word` 기본값 전환 (cssResolver.ts L83 + L286, 2곳)
- [ ] "Normal" 프리셋 제거 + "Break Words" 기본 승격 (TypographySection + deriveTextBehaviorPreset)
- [ ] 기본값 전환 후 회귀 테스트 통과
- [ ] Canvas 2D + WebGL 공존 확인
- [ ] 폰트 로딩 이중 감지 작동
- [ ] 한국어+영어 혼합 Pretext width vs CSS width ≤ 1px
- [ ] CJK+숫자 혼합 줄바꿈 정확
- [ ] Break Hint 텍스트 보존 확인
...
`overflow-wrap: break-word` 기본값 전환 + Pretext SSOT 도입으로 XStudio의 텍스트 정합성과 성능을 동시에 개선한다.

| 측면                        | 현재       | 도입 후 (E2E)       |
| --------------------------- | ---------- | ------------------- |
| CSS↔Skia 줄바꿈 일치율      | ~90%       | ~99%                |
| 리사이즈 재계산 비용        | ~85ms      | ~8ms (10× 개선)     |
| 2-Pass 보정 비용            | ~65ms      | ~10ms (6.5× 개선)   |
| 폰트 파라미터 동기화 포인트 | 3곳 (수동) | 1곳 (자동)          |
| SSOT 적용 범위              | —          | ~100% (기본 텍스트) |
| 추가 번들 크기              | —          | +11~12KB (net)      |

**Phase 0(기본값 전환 + 검증) → A → B → C → D** 순서로 점진적 도입. 각 Phase에 배포 플래그와 되돌림 단위가 있어 독립적으로 rollback 가능. Phase 0 실패 시 대안 경로(B1~B4) 전환.
