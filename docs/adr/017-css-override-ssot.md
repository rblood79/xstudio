...
| ---- | --------------------- | --------------------------------------------------------------------------- | -------------------------- |
| G0   | Phase 2 시작 전       | 자동화 스크립트 dry-run: 치환 대상 목록 확인 + 접두사 충돌 없음 검증        | 스크립트 수정 후 재실행    |
| G1   | Phase 1 완료          | preview-system.css에서 M3 섹션 제거 후 Storybook 시각적 일치                | M3 섹션 복원               |
| G2   | Phase 2 Tier 1~2 완료 | shared 55개 파일 Storybook 전 variant/state 시각적 확인                     | 해당 파일만 revert         |
| G3   | Phase 2 Tier 3 완료   | Builder ITCSS 레이어(canvas/header/footer/panel) 외관 확인                  | 해당 파일만 revert         |
| G4   | Phase 2 Tier 4 완료   | Builder 패널 CSS 52개 전체 + **Publish 앱** 외관 확인                       | 해당 Tier만 revert         |
| G5   | Phase 3 완료          | Canvas에서 Button/Input/Card 렌더링 색상이 Preview와 일치                   | colors.ts만 revert         |
| G6   | Phase 4 완료          | Theme Studio 기본 테마 생성/적용 동작 확인                                  | Theme Studio 변경만 revert |
| G7   | `:focus` 정규화 완료  | 12개 파일에서 `:focus` 제거, `[data-focused]` 통일 + `:focus-visible` 3파일 | 해당 파일만 revert         |
...
> **자동화 권장**: Tier 1 고빈도 토큰 8개는 `sed`/`codemod` 스크립트로 일괄 치환 가능. 스크립트 예시를 Phase 2 시작 전 작성하여 수작업 오류 최소화.
...
```bash
# 자동화 스크립트 예시 (Phase 2 시작 전 작성)
TARGETS="packages/shared/src/components/styles apps/builder/src"
sed -i '' 's/var(--on-surface)/var(--text-color)/g' $(grep -rl 'var(--on-surface)' $TARGETS --include="*.css")
# ... 나머지 토큰도 동일 패턴
# 주의: --primary → --highlight-background 치환 시 --primary-hover, --primary-container 등 접두사 충돌 방지 필요
```
