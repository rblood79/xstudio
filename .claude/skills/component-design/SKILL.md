---
name: component-design
description: MCP 도구를 활용한 컴포넌트 설계/구현 워크플로우. React Aria 문서 참조, IDE 타입 검증, Chrome 브라우저 검증을 단계별로 수행합니다.
user-invocable: true
---

# Component Design Skill (MCP 연동)

MCP 도구를 활용하여 컴포넌트를 설계하고 구현하는 통합 워크플로우입니다.

## 워크플로우 개요

```
1. 리서치 (React Aria MCP)
2. 구현 (Read/Write/Edit + xstudio-patterns)
3. 타입 검증 (IDE MCP)
4. 시각적 검증 (Chrome MCP, 선택)
```

## Phase 1: React Aria 리서치

새 컴포넌트 구현 전 React Aria MCP로 공식 API와 패턴을 조사한다.

### 1-1. 페이지 탐색

```
mcp__react-aria__list_react_aria_pages(includeDescription: true)
```

대상 컴포넌트와 관련된 페이지가 있는지 확인한다.

### 1-2. 페이지 구조 파악

```
mcp__react-aria__get_react_aria_page_info(page_name: "{ComponentName}")
```

섹션 목록을 확인하여 필요한 정보가 어디에 있는지 파악한다.

### 1-3. 상세 문서 참조

```
# API 레퍼런스
mcp__react-aria__get_react_aria_page(page_name: "{ComponentName}", section_name: "Props")

# 사용 예제
mcp__react-aria__get_react_aria_page(page_name: "{ComponentName}", section_name: "Usage")

# 접근성 패턴
mcp__react-aria__get_react_aria_page(page_name: "{ComponentName}", section_name: "Accessibility")
```

### 1-4. 조사 결과 정리

- 사용할 React Aria hooks/components 목록
- Props 인터페이스 설계 기준
- 키보드/접근성 요구사항
- XStudio 컨벤션과의 매핑 포인트

## Phase 2: 구현

xstudio-patterns 스킬의 규칙을 따르며 구현한다.

### 구현 순서 (XStudio 컴포넌트)

1. **타입 정의** — `unified.types.ts`에 Props 타입 추가, `defaultPropsMap` 등록
2. **Spec 작성** — `packages/specs/src/components/`에 Shape 기반 Spec 생성
3. **Factory 정의** — `factories/definitions/`에 생성 팩토리 등록
4. **Preview Renderer** — `preview/renderers/`에 렌더러 추가
5. **Canvas 연동** — `TAG_SPEC_MAP` 등록, `nodeRenderers.ts` 수정 (필요 시)
6. **Property Editor** — 스타일 패널 에디터 추가 (필요 시)

### React Aria 내재화 원칙

- 외부 라이브러리 추가 설치 금지 (번들 500KB 제약)
- React Aria hooks/components는 이미 프로젝트 의존성 — 직접 import 가능
- React Aria 패턴을 XStudio 컨벤션에 맞게 변환:
  - 스타일링 → tv() + CSS
  - 상태 → Zustand 슬라이스
  - 렌더링 → Spec shapes + Skia

## Phase 3: 타입 검증 (IDE MCP)

구현 완료 후 IDE MCP로 타입 진단을 확인한다.

```
mcp__ide__getDiagnostics(uri: "file:///Users/admin/workspace/xstudio/{변경된 파일 경로}")
```

- 에러 0개 확인
- 경고 검토 및 필요 시 수정

## Phase 4: 시각적 검증 (Chrome MCP, 선택)

Storybook이나 개발 서버로 실제 렌더링을 확인한다.

### 5-1. 탭 준비

```
mcp__claude-in-chrome__tabs_context_mcp(createIfEmpty: true)
mcp__claude-in-chrome__tabs_create_mcp()
```

### 5-2. Storybook/Dev 서버 이동

```
mcp__claude-in-chrome__navigate(url: "http://localhost:6006", tabId: {tabId})
# 또는
mcp__claude-in-chrome__navigate(url: "http://localhost:5173", tabId: {tabId})
```

### 5-3. 시각적 확인

```
mcp__claude-in-chrome__computer(action: "screenshot", tabId: {tabId})
```

- 레이아웃 정렬 확인
- 인터랙션 동작 확인
- 반응형 크기 확인

## Phase 생략 조건

| Phase                | 생략 가능 조건                             |
| -------------------- | ------------------------------------------ |
| Phase 1 (React Aria) | React Aria 미지원 컴포넌트, 단순 버그 수정 |
| Phase 3 (IDE)        | `pnpm type-check`로 대체 가능              |
| Phase 4 (Chrome)     | 서버 미실행, 시각적 변화 없는 수정         |
