...
describe("resolveCSSSizeValue - clamp() 함수", () => {
  ...
  it("clamp() 인자 개수 초과 시 fallback", () => {
    expect(
      resolveCSSSizeValue("clamp(100px, 200px, 300px, 400px)", {}, 0),
    ).toBe(0);
  });
...
describe("resolveCSSSizeValue - min() 함수", () => {
  ...
  it("min() 내부에 calc() 중첩 가능", () => {
    expect(resolveCSSSizeValue("min(300px, calc(100% - 100px))", ctx)).toBe(
      300,
    ); // min(300, 700) → 300
  });
...
describe("resolveCSSSizeValue - max() 함수", () => {
  ...
  it("max() 내부에 calc() 중첩 가능", () => {
    expect(resolveCSSSizeValue("max(300px, calc(100% - 600px))", ctx)).toBe(
      300,
    ); // max(300, 200) → 300
  });
...
describe("parseFontShorthand - font-family 다중 fallback", () => {
  it('"italic 16px Arial, Helvetica, sans-serif" — 3개 패밀리', () => {
    const result = parseFontShorthand(
      "italic 16px Arial, Helvetica, sans-serif",
    );
  ...
  it('"bold 14px \\"Noto Sans KR\\", \\"Apple SD Gothic Neo\\", sans-serif"', () => {
    const result = parseFontShorthand(
      'bold 14px "Noto Sans KR", "Apple SD Gothic Neo", sans-serif',
    );
    expect(result?.fontWeight).toBe("bold");
    expect(result?.fontSize).toBe("14px");
...
describe("parseFontShorthand - 개별 속성 우선 (cssResolver 연동)", () => {
  it("font shorthand와 fontStyle 개별 속성이 공존 시 개별 속성 우선", async () => {
    const { resolveStyle, ROOT_COMPUTED_STYLE } =
      await import("../cssResolver");
...
describe("resolveCSSSizeValue - env() 함수", () => {
  ...
  it("env(unknown-name, 50%) with containerSize=800 → fallback 400", () => {
    expect(
      resolveCSSSizeValue("env(unknown-name, 50%)", { containerSize: 800 }),
    ).toBe(400);
  });
});
