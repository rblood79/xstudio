...
Proposed — 2026-04-16 (Codex 리뷰 1차 반영 2026-04-17 — Q1 정정 + Q6/Q7/Q8 신설 + 데이터 모델 정정 + Preview 경로 수정. Codex 리뷰 2차 반영 2026-04-17 — Q8 패키지 의존성 정정(specs SSOT) + Stored/Runtime 분리 + Q9/Q10 신설 + index.css 수동 import 명시)
...
import { resolveActionId } from "../events/EVENT_REGISTRY"; // ADR-055

const stored = (element.props.items ?? []) as StoredMenuItem[];
// Stored → Runtime 1회 변환 (onActionId → 함수)
const runtime: RuntimeMenuItem[] = stored.map((it) => ({
  ...it,
  onAction: it.onActionId ? resolveActionId(it.onActionId) : undefined,
  children: it.children?.map(/* 재귀 변환 */),
}));
return (
  <MenuButton
    key={element.id}
...
- **per-item event 직렬화 필요** (Q7=ii) — `onAction: () => void`는 store 직렬화 불가 → `StoredMenuItem.onActionId: string`(EVENT_REGISTRY 참조) + `RuntimeMenuItem.onAction: () => void` 두 인터페이스 분리. 변환은 `CollectionRenderers.tsx:renderMenu`에서 1회 (Codex 2차 정정)
