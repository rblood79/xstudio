...
- **G1/G2 snapshot 이중 유지 — 동시 업데이트 의무**: `tokenSnapshot.test.ts`(primitives G1)와 `tokenConsumerDrift.test.ts` C2(resolveToken G2)가 동일 resolved 값을 각각 snapshot으로 고정. 역할은 다르나(primitives 변경 감지 vs resolveToken 매핑 오류 감지) primitives 변경 시 `--update-snapshots`을 두 파일에서 동시 실행해야 함 — 어느 하나만 업데이트 시 CI 불일치 잔존. PR에서 두 snap 파일 동시 변경 여부 확인 필요 (ADR-081 패턴)
...
- **폰트 레지스트리 동기화 useEffect 복제**: `composition:custom-fonts-updated` + `StorageEvent(FONT_REGISTRY_STORAGE_KEY)` 이중 채널 구독 패턴이 `FontManagerPanel.tsx`와 `TypographySection.tsx`에 동일하게 복제 — `useFontRegistry()` 커스텀 훅으로 추출 필요
