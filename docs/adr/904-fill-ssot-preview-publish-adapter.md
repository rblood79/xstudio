...
>
> **구현 전이 근거**: Phase 1~5 코드 land 완료. `fillMigration` read-through, `inspectorActions` write-through helper 수렴, shared `fillAdapter` 기반 Preview/Publish/Inspector consumer 정렬, `VITE_FEATURE_FILL_V2` rollout/rollback 기준선 문서화, Style Panel/store/update sink/AI/clipboard/import/add-element/preview-generated batch ingress canonicalization까지 반영했다. 잔여 이슈는 본 ADR scope 밖의 "임의 비정형 background payload 정책"과 component-specific `backgroundColor` 예외 구분이며, 핵심 Fill SSOT + adapter 전환은 구현 완료로 본다.
...
2. **하위 호환성**: 기존 `style.backgroundColor`만 가진 프로젝트 로드 성공률 **100% 유지**. 영향 범위 수식화 —
   ...
   - 롤백 보장: `VITE_FEATURE_FILL_V2=false` 시 기존 경로 그대로 동작, 신규 필드 무시.
...
| R4  | 전환 기간 동안 Fill + CSS 파생 필드 이중 저장으로 유지보수 부담 누적                                                                     |  MED   | G5 — Phase 5 에서 신규 문서 저장은 파생 필드화, 이중 저장 완전 제거 시점은 후속 ADR 로 분리                                      |
...
| G2: Write-through  | R2         | Phase 2 | Fill 편집 시 `fills` 저장 + CSS 파생 필드 동기화 성공, undo/redo 회귀 0          | write-through 범위 축소               |
...
| G5: Legacy 축소    | R4         | Phase 5 | 신규 생성 문서의 background 직렬화 의존도 제거(파생 필드화)                      | 이중 저장 유지                        |
...
- P2 Write-through 구현: Fill 편집 commit/preview 경로가 공용 helper 로 CSS 파생 필드를 동기화하고 undo/redo 회귀 테스트를 통과한다.
...
- P4 Rollout 기준선 구현: `VITE_FEATURE_FILL_V2` off 시 Builder 편집 surface 와 Skia direct-read 만 legacy fallback 으로 내려가고, Preview/Publish adapter 는 flag 비의존 경로로 유지된다.
- P5 Legacy 축소 구현: Style Panel direct background edit 차단, Modified Styles read-only 전환, AI tool surface `fills` 입력 수용, store/update sink 정리, clipboard/import/add-element/preview-generated batch canonicalization 반영.
- 후속 분리: 임의 비정형 `backgroundImage`/data URL payload 정책과 `RowEditor`/`CellEditor` 같은 component-specific `backgroundColor` 도메인 분리는 별도 후속 결정 항목이다.
