...
ADR-023과 ADR-030을 통해 S2 컴포넌트와 전용 에디터 기반은 마련되었지만, Property Editor의 프로퍼티 노출 범위가 React Spectrum S2 문서와 현재 XStudio 계약을 기준으로 일관되지 않다.

이번 문제는 단순히 "에디터에 스위치 몇 개를 더 추가"하는 수준이 아니다. 현재 코드베이스에는 아래 두 종류의 갭이 섞여 있다.

1. 이미 XStudio 계약에 존재하지만 에디터에서 노출하지 않는 갭
2. S2 문서에는 존재하지만 XStudio 타입, shared 컴포넌트, renderer, spec, preview 경로에 아직 계약이 없는 갭
...
- 반대로 아래 항목은 현재 XStudio 계약에 아직 없다.
...
- 하지만 그 전체를 이번 ADR 범위에 포함하면 "Property Editor 정렬"이 아니라 "XStudio 컴포넌트 계약 확장" ADR이 된다.
- 따라서 이번 결정은 S2 문서 전체를 그대로 노출하는 것이 아니라, **현재 XStudio 계약과 실제 렌더 경로가 지원하는 범위부터 정렬**할지 여부를 다뤄야 한다.
...
- 설명: 현재 XStudio 타입과 shared component, renderer 경로가 이미 수용하거나 짧은 범위에서 동기화 가능한 prop만 우선 Property Editor에 노출한다. 계약이 없는 S2 prop는 별도 후속 작업으로 분리한다.
- 장점:
  - ADR 범위가 명확하다.
...
1. 현재 XStudio 계약에 이미 존재하거나, 같은 변경 세트 안에서 타입/spec/renderer 동기화가 명확히 가능한 prop만 Property Editor에 노출한다.
2. S2 문서에는 존재하지만 현재 XStudio 계약에 없는 prop는 "계약 확장 후보"로 분리하고, 별도 ADR 또는 후속 breakdown에서 다룬다.
...
- [ADR-045 Breakdown](/Users/admin/work/xstudio/docs/design/adr-045-s2-property-editor-alignment-breakdown.md): 컴포넌트별 실행 메모와 계약 확장 후보
- [React Spectrum S2 Dialog](/Users/admin/work/xstudio/.agents/skills/react-spectrum-s2/references/components/Dialog.md)
...
- [React Spectrum S2 Link](/Users/admin/work/xstudio/.agents/skills/react-spectrum-s2/references/components/Link.md)
- [React Spectrum S2 Menu](/Users/admin/work/xstudio/.agents/skills/react-spectrum-s2/references/components/Menu.md)
- [React Spectrum S2 Tabs](/Users/admin/work/xstudio/.agents/skills/react-spectrum-s2/references/components/Tabs.md)
- [React Spectrum S2 Popover](/Users/admin/work/xstudio/.agents/skills/react-spectrum-s2/references/components/Popover.md)
- [React Spectrum S2 Tooltip](/Users/admin/work/xstudio/.agents/skills/react-spectrum-s2/references/components/Tooltip.md)
- [React Spectrum S2 Breadcrumbs](/Users/admin/work/xstudio/.agents/skills/react-spectrum-s2/references/components/Breadcrumbs.md)
