...
describe("토큰-CSS 변수 매핑 일관성", () => {
  describe("color 토큰", () => {
    ...
    it.each(colorKeys)("color.%s → 유효한 CSS 표현식", (key) => {
      const ref = `{color.${key}}` as TokenRef;
      const cssVar = tokenToCSSVar(ref);
      // var(--name) 또는 color-mix(...) 형태 모두 유효
      expect(cssVar).toMatch(/^(var\(--[\w-]+\)|color-mix\(.+\))$/);
    });

    it.each(colorKeys)(
      "color.%s → resolve 결과가 프리미티브와 일치 (light)",
      (key) => {
        const ref = `{color.${key}}` as TokenRef;
  ...
  describe("spacing 토큰", () => {
    ...
    it.each(spacingKeys)(
      "spacing.%s → resolve 결과가 프리미티브와 일치",
      (key) => {
        const ref = `{spacing.${key}}` as TokenRef;
        const resolved = resolveToken(ref, "light");
  ...
  describe("typography 토큰", () => {
    ...
    it.each(typographyKeys)(
      "typography.%s → resolve 결과가 프리미티브와 일치",
      (key) => {
        const ref = `{typography.${key}}` as TokenRef;
        const resolved = resolveToken(ref, "light");
  ...
  describe("radius 토큰", () => {
    ...
    it.each(radiusKeys)(
      "radius.%s → resolve 결과가 프리미티브와 일치",
      (key) => {
        const ref = `{radius.${key}}` as TokenRef;
        const resolved = resolveToken(ref, "light");
  ...
  describe("shadow 토큰", () => {
    ...
    it.each(shadowKeys)(
      "shadow.%s → resolve 결과가 프리미티브와 일치",
      (key) => {
        const ref = `{shadow.${key}}` as TokenRef;
        const resolved = resolveToken(ref, "light");
