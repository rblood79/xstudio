...
- DesignKit role 마커 (재사용 원본 시각화) **0건**
...
1. **5 요소 land 필수** (ADR-903 Hard Constraint #10):
   - ① reusable / ref / override 시각 마커 3종 (LayerTree + Canvas + DesignKit)
   - ② 원본 ↔ 인스턴스 양방향 탐색 액션
...
| ------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| **G4-A**: 시각 마커 3종 land          | Phase A | (a) LayerTree/Canvas/DesignKit 에서 reusable/ref/override 각각 고유 시각 마커 / (b) Storybook 시각 검증 / (c) 기존 시각 cue 와 충돌 0                     | 디자인 재검토 + low-friction 마커 fallback   |
