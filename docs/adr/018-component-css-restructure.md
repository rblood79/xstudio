...
ADR-017에서 M3 토큰을 제거하고 시맨틱 토큰 + Tailwind로 전환하더라도, 컴포넌트 CSS **구조 자체**의 비효율은 그대로 남는다. react-aria-starter의 원본 CSS 패턴과 비교하면 XStudio 컴포넌트 CSS는 과도하게 비대하다.

**정량적 비교:**

| 지표          | react-aria-starter | XStudio 현재      | 배율     |
| ------------- | ------------------ | ----------------- | -------- |
| 총 CSS 라인   | 4,430줄 (56파일)   | 15,652줄 (79파일) | **3.5x** |
...
XStudio Button은 7 variant × 3 state = 21개 셀렉터 블록이 명시적으로 선언되어 있다:

```css
/* XStudio 현재 — 각 variant마다 bg/color/border를 3번씩 반복 */
...
→ utilities.css의 `[data-variant]` 셀렉터와 비호환. Phase 5에서 **Card variant를 `data-variant`로 마이그레이션** 필요 (React 컴포넌트 + CSS 동시 변경).
...
1. **ADR-017 선행 필수**: M3 토큰 제거 완료 후 진행
...
6. **Card.css data-variant 마이그레이션**: 클래스 기반 → data-variant 기반 전환 포함

## Alternatives Considered

### 대안 A: Starter 완전 채택 (Full Starter Alignment)

- **설명**: starter CSS 56개 파일을 원본 그대로 도입. XStudio 전용 확장(size variant 등)만 별도 레이어로 추가.
- **장점**: 원본 유지 → 업스트림 업데이트 추적 용이, 코드량 최소
- **단점**: oklch relative color syntax 브라우저 호환성 이슈 (Safari 16.3 이하, Firefox 127 이하 미지원), XStudio 고유 기능(7 variant, 5 size) 삭제 또는 별도 관리
- **위험**: 기술(M) / 성능(L) / 유지보수(L) / 마이그레이션(H)

### 대안 B: 선택적 채택 — utilities 패턴 + 구조 단순화

- **설명**: starter의 `utilities.css` 3대 유틸리티(button-base, indicator, inset) 패턴을 도입하되, oklch 대신 XStudio 시맨틱 토큰으로 파생 변수 구현. 컴포넌트 CSS는 구조(레이아웃/크기)만 남기고 시각적 스타일은 utility에 위임.
...
4. **기존 인터페이스 유지**: `data-variant`, `data-size` 속성 그대로 → React 컴포넌트 변경 없음
...
starter의 3대 유틸리티를 XStudio 시맨틱 토큰 기반으로 재구현:
...
**목표**: starter의 3대 유틸리티를 XStudio 시맨틱 토큰 기반으로 구현
...
| ---------------------- | :--------: | :--------: | :----: |
| Button.css             |    186     |    ~50     |  -73%  |
| ToggleButton.css       |    148     |    ~40     |  -73%  |
| Breadcrumbs.css        |     95     |    ~30     |  -68%  |
| Link.css               |     78     |    ~30     |  -62%  |
| Toolbar.css            |     68     |    ~25     |  -63%  |
| SegmentedControl.css\* |     —      |     —      |  신규  |
| TagGroup.css           |    178     |    ~60     |  -66%  |
...
### Phase 4: Tier 3 컴포넌트 전환 — 인디케이터류 (6파일)

**목표**: `.indicator` 유틸리티 적용

| 컴포넌트        | 현재 줄 수 | 예상 줄 수 | 감소율 |
| --------------- | :--------: | :--------: | :----: |
| Checkbox.css    |    287     |    ~60     |  -79%  |
| RadioGroup.css  |    178     |    ~50     |  -72%  |
| Switch.css      |    230     |    ~60     |  -74%  |
| Slider.css      |    339     |    ~80     |  -76%  |
| ProgressBar.css |    137     |    ~40     |  -71%  |
| Meter.css       |    112     |    ~35     |  -69%  |

### Phase 5: Tier 4 컴포넌트 전환 — 복합/오버레이류 + 레거시 패턴 (15+파일)

**목표**: 나머지 컴포넌트 정리 (Popover, Dialog, Menu, ListBox, Table, Tabs 등) + Card.css 셀렉터 마이그레이션

| 컴포넌트 그룹                                     | 파일 수 | 현재 총 줄 수 | 예상 감소율 |
| ------------------------------------------------- | :-----: | :-----------: | :---------: |
| 오버레이 (Popover, Dialog, Modal, Tooltip, Toast) |    5    |     ~780      |    -50%     |
| 컬렉션 (ListBox, Menu, GridList, Table, Tree)     |    5    |    ~2,804     |    -40%     |
| 탭/네비 (Tabs, Breadcrumbs, Disclosure)           |    3    |     ~813      |    -50%     |
| 기타 (Calendar, Form, Content 등)                 |   5+    |     ~800      |    -40%     |

**Card.css 셀렉터 마이그레이션 (이 Phase에서 함께 수행):**

Card.css의 클래스 기반 variant (`.primary`, `.elevated` 등)를 `[data-variant]` 기반으로 전환:

- **CSS**: `&.primary` → `&:where([data-variant="primary"])`
- **React**: Card.tsx에서 variant prop을 `data-variant` 속성으로 전달하도록 변경
- 이 작업은 Card 컴포넌트 인터페이스는 유지하되, 내부 구현만 표준화

**Builder CSS 참고**: Builder 전용 CSS 7파일(`canvas.css`, `header.css`, `footer.css`, `panel-nav.css`, `panel-container.css`, `error-loading.css`, `themes/index.css`)은 ADR-017에서 M3 토큰 치환만 수행. ADR-018의 utilities 패턴은 Builder CSS에 적용하지 않음 (builder-system.css 독립 유지).
...
| 지표                    | Before |        After         |   변화   |
| ----------------------- | :----: | :------------------: | :------: |
| 총 CSS 라인             | 15,652 |        ~6,000        | **-62%** |
| 파일 수                 |   79   | ~80 (+utilities.css) |    ±0    |
| variant 오버라이드 블록 | ~100+  |  ~15 (utilities 내)  | **-85%** |
| 컴포넌트별 평균 줄 수   |  ~200  |         ~60          | **-70%** |
...
1. **CSS 62% 감소** (15,652 → ~6,000줄) — 번들 크기 + 파싱 시간 개선
...
4. **Card.css 셀렉터 마이그레이션** — 클래스 기반(`.primary`) → `[data-variant]` 기반 전환. React Card.tsx도 수정 필요
