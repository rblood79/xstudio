...
> - revision 3 (2026-04-28 세션 49 후속): codex 3차 review 통과 (M-1 의존 방향 정정 / L-1 LOW 추적성 권고) + **의존 방향 정정 — ADR-911 = 본 ADR 의 frame-bundled preset 응용 ADR**. baseline (ADR-903 Phase 4) framing 이 거꾸로 박혀 있던 것을 정정. ADR-911 Phase 3 후속 동결 (재개 = 본 ADR Phase A1 land 후), 본 ADR 이 component 추상의 base, ADR-911 이 frame 응용 specialization. Status `Proposed → Accepted` 승격
...
2. **Properties 패널 ##Component section## 위치 (revision 2 결정)**:
   ...
   - Slot 편집 (`frame.slot: false | string[]` schema) 은 별도 ##Slot section## — ADR-911 영역. **ADR-911 ##Slot section## 은 본 ADR ##Component section## 의 frame-specific specialization** — 본 ADR 이 component 추상의 base UI 를 정의하고, ADR-911 이 frame (= layout-bundled reusable component) 의 slot 편집 specialization 을 정의 (revision 3 의존 방향 정정)
...
| R5  | ADR-911 (Layout/frameset preset 응용) 의 신 FramesTab / ##Slot section## 가 본 ADR ##Component section## 와 충돌 (revision 3 의존 방향 정정 — ADR-911 = preset 응용 / 본 ADR = base)                                                                                           |                                                          MED                                                          | 본 ADR Phase A1 (시각 마커 base) 선행 land → ADR-911 Phase 3 후속 (P3-ε / P3-ζ) 가 본 ADR ##Component section## 위 frame-specific specialization 으로 재설계 후 진입. ADR-911 ##Slot section## 이 본 ADR ##Component section## 의 frame 응용. 충돌 0 설계 — 별도 section 공존, cross-section reference 만 link 형태                                                                                                                                                                         |
...
- 본 ADR ##Component section## (component 추상 base) + ADR-911 ##Slot section## (frame-bundled preset specialization) 의 직교 공존 — pencil app 호환 frame authoring 위에 component 편집 경험 통합 (revision 3: 본 ADR base / ADR-911 preset 응용 framing)
...
- [ADR-911](911-layout-frameset-pencil-redesign.md) — Layout/frameset pencil 호환 재설계. **본 ADR 의 frame-bundled preset 응용 ADR** (revision 3 의존 방향 정정 — 선행 ADR 아님). 본 ADR 이 reusable component 추상의 base UI 를 정의하고, ADR-911 이 frame (= layout-bundled reusable component) 의 frame-specific specialization 을 정의. ADR-911 ##Slot section## 은 본 ADR ##Component section## 의 frame 응용 specialization. ADR-911 Phase 3 후속 (P3-ε / P3-ζ) 은 본 ADR Phase A1 land 후 재개
