...
In Progress — 2026-05-02 direct cutover 반영 (P3-ε/P3-ζ closure + G3 cascade slices #1~#3 유지, legacy flag/migration/panel-slot 명칭 정리 완료)
...
- **2026-05-02 — ADR-916 projection removal follow-up**: FramesTab/PageLayoutSelector/ComponentsPanel 과 BuilderCanvas/canvas drag-drop read path 는 active canonical document 를 사용하고, `selectCanonicalDocument()` projection rebuild 를 더 이상 수행하지 않는다. 초기 hydration 전 visible list 는 `layouts` fallback 으로 유지한다. 검증: targeted vitest 8 files / 25 tests PASS.
- **2026-05-02 — ADR-916 projection removal follow-up #2**: BuilderCore frame refresh/publish, usePageManager hydrate, elementCreation history/reorder, layoutActions slot read caller 경로도 active canonical document 또는 frame mirror adapter 로 전환했다. caller/store action 레벨의 `selectCanonicalDocument()` rebuild 는 제거했고, 잔여는 sync/adapter boundary 로 축소했다. 검증: targeted vitest 12 files / 60 tests PASS.
