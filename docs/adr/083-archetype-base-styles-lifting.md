...
Proposed — 2026-04-20 (**Revision 3**: Codex Round 3 HIGH 반영 — Skia consumer 일반화를 Phase 0 으로 본 ADR scope 에 포함)
...
3. 기존 Skia 렌더 회귀 0 — Phase 0 consumer 일반화 시 기존 `containerTag === "..."` 하드코딩 분기들과 우선순위 충돌 없음 감사 필수 (하드코딩 분기는 대부분 `parentStyle.display ?? ...` 형태이므로, spec fallback 이 parentStyle 에 선주입되면 자연 override). 하드코딩 분기 해체는 본 ADR scope 외 (후속)
...
- 설명: Phase 0 으로 `CONTAINER_STYLES_SPEC_MAP` 을 TAG_SPEC_MAP 기반 조회로 일반화 + `CONTAINER_STYLES_FALLBACK_KEYS` 를 8 필드로 확장. 이후 Phase 1-11 로 11 archetype 의 63 spec 에 layout primitive `containerStyles` 선언. `ARCHETYPE_BASE_STYLES` 테이블 유지.
...
| ID  | 위험                                                                                                                                                           | 심각도 | 대응                                                                                                                                                                                                                                                   |
| :-: | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R0  | **Phase 0 consumer 일반화 시 기존 `containerTag === "..."` 하드코딩 분기와 우선순위 충돌** — spec fallback 이 하드코딩 값을 override 하거나 역으로 덮일 가능성 |  MED   | Phase 0 착수 전 하드코딩 분기 전수 감사 (grep `containerTag === "` → 태그별 값 비교). 대부분 `parentStyle.X ?? "기본값"` 패턴이라 parentStyle 에 spec fallback 을 **먼저** 주입하면 자연 override. 감사 결과 충돌 발견 태그는 whitelist 에서 일시 제외 |
...
| :--: | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
|  G0  | **Phase 0 완료**       | (a) `CONTAINER_STYLES_SPEC_MAP` 일반화 + `CONTAINER_STYLES_FALLBACK_KEYS` 8 필드 확장. (b) 기존 `containerTag === "..."` 하드코딩 분기 전수 감사 — 충돌 없음 확인 (R0). (c) Chrome MCP: **기존 리프팅된 ListBox 외 최소 1 태그** (InlineAlert 는 이미 하드코딩 있음 → 다른 태그 선택, 예: Select trigger 또는 임시 spec.containerStyles 주입 테스트) 에서 spec 값이 Skia 에 반영됨 확증. (d) `pnpm type-check` 3/3 PASS | Phase 0 revert, scope whitelist 축소 후 재진입 |
