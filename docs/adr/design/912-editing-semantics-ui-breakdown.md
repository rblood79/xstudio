...
| Frames mode 에서 frame 이 Page 와 다른 위치/크기로 생성됨 | Frames mode 를 별도 authoring area 가 아니라 현재 Page 작업면 전환으로 정렬. `frameAreas` 를 current page position + `pageWidth/pageHeight` 로 정규화하고 layout publish/render root 는 stale `framePositions` 를 무시 |
