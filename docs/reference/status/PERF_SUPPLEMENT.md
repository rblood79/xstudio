...
> **작성자**: Antigravity AI
> **작성일**: 초안(2025-12-09) | **최종 수정**: 2025-12-11 (Phase 10 결정으로 Publish App용으로 재분류)

본 문서는 기존 최적화 계획에서 식별된 공백을 보완하기 위한 추가 제안 사항입니다.

> **⚠️ Phase 10 (WebGL Builder) 결정에 따른 변경**
> - Builder는 WebGL 기반으로 전환되므로 DOM 가상화가 불필요
> - 아래 DOM 최적화 항목들은 **Publish App** (React DOM 기반)에 적용
> - Web Worker는 Builder/Publish App 모두에 적용 가능

---

## 1. 캔버스 가상화 (Canvas Virtualization) - 📦 Publish App용
...
## 3. 에셋 최적화 (Asset Lazy Loading) - 📦 Publish App용
...
> **⚠️ Phase 10 (WebGL Builder) 결정 이후 재분류됨**

| 우선순위      | 항목              | 적용 대상 | 구현 난이도 | 예상 효과                          | 비고                               |
| :------------ | :---------------- | :-------- | :---------- | :--------------------------------- | :--------------------------------- |
| **📦 Publish** | **캔버스 가상화** | Publish App | 상          | DOM 렌더링 성능 | Builder는 WebGL로 대체 |
| **P1 (권장)** | **웹 워커**       | Builder + Publish | 상          | UX 반응성(Responsiveness) 확보     | 양쪽 모두 적용 가능       |
| **📦 Publish** | **에셋 최적화**   | Publish App | 하          | 초기 로딩 속도 개선                | Publish App SSR/SSG에 필수           |

---

## 5. 결론

**Phase 10 (WebGL Builder) 결정에 따라:**

1. **Builder**: WebGL 기반으로 전환 → DOM 가상화 불필요, PixiJS Culling으로 대체
2. **Publish App**: React DOM 기반 유지 → 캔버스 가상화, 에셋 최적화 적용
3. **Web Worker**: Builder/Publish App 모두에 적용 가능 (Diff, Index 연산)

자세한 Builder 최적화는 [10-webgl-builder-architecture.md](./10-webgl-builder-architecture.md) 참조.
