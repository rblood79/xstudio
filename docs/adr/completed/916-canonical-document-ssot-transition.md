...
- **2026-05-03 — post-cutover persistence / canonical frame scope fix**:
  ...
  - Layout Preset 변경 시 Slot 이 계속 누적되던 후속 회귀도 같은 canonical write-through gap 으로 닫았다. `removeElements` 는 삭제 후 `setElementsCanonicalPrimary()` 로 full snapshot 을 canonical document 에 반영하고, `updateElementProps` 는 body `appliedPreset` / container style 변경을 `mergeElementsCanonicalPrimary()` 로 반영한다.
  - Style Panel Layout section 의 frame body / Slot style 변경이 Skia frame render 에 반영되지 않던 회귀도 같은 원인이다. Style Panel commit/preview 경로는 `inspectorActions` 에서 별도로 처리되어 `updateElementProps` canonical write-through 를 우회했으므로, 해당 경로도 active `CompositionDocument` 에 즉시 merge 하고 commit 시 documents store persistence 를 수행하도록 고정했다.
  - `gap` batch edit 은 store longhand 정책에 맞춰 `rowGap` / `columnGap` 으로 분배하고, reset dirty 판정도 longhand 값을 읽도록 보강했다.
  - 검증: targeted builder vitest 11 files / 83 tests PASS, Layout Preset canonical mutation targeted vitest 8 files / 60 tests PASS, Style Panel canonical mutation targeted vitest 4 files / 49 tests PASS, browser smoke 에서 page Button add/reload 유지 + frame Slot add/reload 유지 + immediate frame Layers `["body", "Slot: content"]` + frame body/Slot style store/canonical 동시 갱신 + unexpected relevant console/page errors 0건, `pnpm run codex:preflight` PASS.
