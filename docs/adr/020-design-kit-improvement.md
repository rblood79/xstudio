...
Design Kit 패널은 XStudio의 디자인 시스템 패키징/공유 기능이다. 디자인 변수, 테마/토큰, Master 컴포넌트를 하나의 `.kit.json` 파일로 묶어 프로젝트 간 재사용할 수 있게 한다.
...
**현상:** 내장 킷 1개, 컴포넌트 2개 (Card, Badge). Card는 `Box + 2개 Text`로 구성 — XStudio의 실제 Card 컴포넌트(CardHeader + CardContent Compositional Architecture)와 무관.

**영향:** 사용자가 패널을 열어도 실질적으로 쓸 것이 없다. "Basic Kit"의 Card는 `Box` 태그에 인라인 스타일을 넣은 것일 뿐, XStudio의 `Card` 컴포넌트가 아니다.

**근본 원인:** Kit 컴포넌트 스키마(`KitElement`)가 XStudio의 Compositional Architecture (factory definitions, TAG_SPEC_MAP, 46+ 컴포넌트)와 분리되어 있음. 킷은 자체 `tag: 'Box'` 기본 요소만 사용하고, `tag: 'Card'`나 `tag: 'Button'` 같은 XStudio 네이티브 컴포넌트를 활용하지 않는다.

### 2.2 XStudio 컴포넌트 시스템 미활용 (HIGH)

**현상:** Kit이 생성하는 요소는 `Box`, `Text` 기본 태그만 사용. XStudio가 9개 카테고리에 46+ 컴포넌트를 제공하고, 각각 Factory Definition과 Spec이 있음에도 이를 전혀 활용하지 않는다.

| Kit 현재 사용 | XStudio 실제 보유                         |
| ------------- | ----------------------------------------- |
| Box, Text     | Button, Input, Select, Card, Tabs, Badge, |
|               | Table, Calendar, DatePicker, Modal,       |
|               | Accordion, ToggleButtonGroup 등 46+ 종류  |

**영향:** Kit으로 만든 컴포넌트는 XStudio의 Spec 렌더링, Skia 캔버스 렌더링, 스타일 패널 에디터 연동이 제대로 작동하지 않는다.
...
| 2   | XStudio 컴포넌트 시스템 미활용 | HIGH     | 렌더링/스펙/에디터 | HIGH        |
...
- 유지보수 MEDIUM: 킷 스키마가 XStudio 컴포넌트와 분리된 구조적 문제가 남음

### 대안 B: Kit 스키마 v2 전면 재설계

Kit JSON 스키마를 XStudio의 Factory Definition + Compositional Architecture에 맞게 재설계.

- **설명:** `KitElement`를 XStudio의 `ChildDefinition` (factory) 포맷으로 교체. Kit 컴포넌트가 `tag: 'Card'`를 쓰면 자동으로 CardHeader + CardContent Compositional 구조가 생성되도록 Factory 시스템과 통합.
- **장점:** XStudio 네이티브 컴포넌트 완전 활용, Spec/렌더링/에디터 자동 연동
...
- **설명:** `.pen` 파일의 `get_variables`, `set_variables`, `get_style_guide` 개념을 XStudio 킷에 적용. 킷이 변수/토큰/컴포넌트 묶음이 아닌, 완전한 스타일 가이드(Typography Scale, Color Palette, Spacing System, Component Library)를 포함.
...
대안 A를 기반으로 하되, 대안 B의 핵심인 **XStudio Factory 통합**을 Phase 2에서 도입한다. 이를 통해:

1. 즉시 가치를 제공할 수 있는 개선 (History, UI, 내장 킷)을 빠르게 적용
2. 구조적 문제 (Kit ↔ XStudio 컴포넌트 분리)를 Phase 2에서 해결
...
### Phase 2: XStudio 컴포넌트 통합 (Kit v2 스키마)

```
P2-1: Kit 스키마 v2 설계 — KitElement에 XStudio tag 연동, Factory ChildDefinition 호환
P2-2: v1 → v2 자동 마이그레이션 — validateKitJSON에 버전 감지 + 변환 추가
P2-3: Factory 통합 — Kit 컴포넌트가 createFromFactory() 활용
P2-4: 내장 킷 확충 — XStudio 네이티브 컴포넌트 활용 킷 5~10개
...
**핵심 변경:** `KitElement`가 XStudio의 Factory `ChildDefinition`과 호환.

```typescript
// v2 KitElement
interface KitElementV2 {
  localId: string;
  tag: string; // XStudio 네이티브 태그 사용 ('Card', 'Button' 등)
  ...
  components: [
    {
      master: {
        localId: 'stat-card',
        tag: 'Card',           // XStudio 네이티브 Card 컴포넌트
...
- **Phase 2 구조 개선**: XStudio 네이티브 컴포넌트 활용으로 킷 품질 비약적 향상
...
- **내장 킷 유지보수**: XStudio 컴포넌트 변경 시 내장 킷도 함께 업데이트 필요
