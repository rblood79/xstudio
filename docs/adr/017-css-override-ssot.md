...
**영향 범위**: shared CSS 79개 파일 + Builder CSS 14개 파일 = **총 93개 CSS 파일**
...
| `--primary-hover/pressed`     | `color-mix()` 유지 또는 `--highlight-background-pressed` | 이미 존재                    |
...
| 마이그레이션 | **MEDIUM** | 88개 파일 find-and-replace 성격. 레이아웃 미영향. 단계적 적용 가능 |
...
### Phase 2: 컴포넌트 CSS M3 토큰 치환 (88파일)

각 컴포넌트 CSS에서 M3 토큰 → 시맨틱/Tailwind 치환. 우선순위:
...
**Tier 3 — Builder CSS 정리:**

builder-system.css의 M3 섹션 제거 후, Builder 영역 14파일에서 잔여 M3 참조를 시맨틱/Tailwind로 치환.
...
3. `:focus` 사용 금지 규칙 (`[data-focused]` 통일)
